...
% \autoref to an appendix section must read "Supplementary Materials S#".
% Set here (right after \begin{document}, so after hyperref's and babel's
% \AtBeginDocument setup) so it applies to the BODY footnotes that reference
% appendix sections too -- \autoref resolves the name at the point of use, and
% hyperref uses \appendixautorefname for a \section that appears after \appendix.
\renewcommand{\sectionautorefname}{Supplementary Materials}
\renewcommand{\subsectionautorefname}{Supplementary Materials}
\renewcommand{\appendixautorefname}{Supplementary Materials}
...
% =====================================================================
\begin{abstract}
    \noindent We document that \textit{apex} corruption scandals---corrupt acts implicating top-level officials---cause immediate and persistent increases in violent protests. Using a panel of 174 corruption scandals across 16 countries in Latin America from 2008 to 2019, merged with daily protest counts, we show that the daily count of violent protests rises sharply after scandals implicating Apex officials---by about 0.051 protests per country-day in the month following disclosure, roughly 2.6 times the pre-scandal baseline---while scandals implicating Non-Apex officials produce no such immediate response, and we reject the equality of the two effects. The response appears within 15 days of disclosure and persists even after 60 days. The effect is specific to apex corruption: neither football match losses nor currency depreciations, two other high-salience national shocks, move violent protests. Our findings identify a high-frequency, high-magnitude channel through which corruption shapes the political environment in democracies.
\end{abstract}
...
Our empirical analyses exploit the randomness of corruption scandal disclosure dates in Latin American news outlets between 2008 and 2019. In particular, we use an interrupted time-series design in which we compare protest occurrences in countries with scandals before and after the scandal occurs. Our analysis dataset is created by: (i) Scraping the Twitter accounts from news outlets in Latin America to identify the calendar date in which a scandal in a given country is first reported\footnote{We keep the date of the first news report to (a) be able to estimate the effect of a scoop, and (b) avoid double-counting a scandal due to follow-up news reports on it.}, (ii) create an event-window of $T \in \{30, 60, 90\}$ days before and after the scandal, (iii) merge the country-day count of protests from the MM data to create a `scandal-protest' dataset for a given country in a given calendar-date window, and lastly, (iv) append all such scandal-protest datasets to build the analysis dataset.

The MM dataset is available for the period 1990--2020, and it includes information on the date, size, and location of any episode in which 50 or more protesters publicly demonstrate against the State.  If the same protest is associated with more than one date (e.g., because the news source for the protest cannot accurately identify whether it is a continued or a new protest), then we keep the date of the earliest protest. If a protest lasts $X$ days, we code the occurrence of a protest at dates $t, t+1, \dots, t+X-1$. For a protest to be classified as ``violent'', ``[t]he violence could include anything from riotous behavior that destroys property to shooting at the police or military''~\citep{clark2016mass}\footnote{In \autoref{sup:protests_examples} we illustrate a few of the protests in our dataset.}. Our main outcome is the number of violent or peaceful protests occurring in a given country-date. We focus on the years 2008--2019, given that our corruption scandals data is available for those years.

Importantly, our recollection of scandals allows us to identify the position of the implicated official. This further allows us to manually classify scandals in two categories: (i) ``Apex'' includes 63 scandals in which the implicated official is a President (current or former), a Governor, or a Supreme Court Justice, and (ii) ``Non-Apex'' includes 111 scandals in which the implicated official is a Congressman, lower judiciary, or other lower-rank officials\footnote{See \autoref{sup:classification} for more information on our sample of scandals and their classification.}.

We use Ordinary Least Squares (OLS) to estimate the following equation:
\begin{equation}
\label{eq:main}
    Y_{ct} \;=\; \beta_{\text{Apex}}\, \mathrm{Post}_{st}\, \mathbf{1}\{\text{Apex}_{c(s)}\}
    \;+\; \beta_{\text{Non-Apex}}\, \mathrm{Post}_{st}\, \mathbf{1}\{\text{Non-Apex}_{c(s)}\}
    \;+\; \alpha_d + \lambda_m + \theta_{cy} + \varepsilon_{ct},
\end{equation}
where $\alpha_d$ denote day-of-the-week fixed effects, $\lambda_m$ denote month-of-the-year fixed effects, $\theta_{cy}$ denote country-year fixed effects, $\text{Post}_{st}$ is an indicator for whether date $t$ is after the disclosure date of scandal $s$, $\mathbf{1}\{\text{(Non-)Apex}_{c(s)}\}$ is an indicator for whether scandal $s$ in country $c$ implicates an (non-)apex official, $Y_{ct}$ denotes the outcome of interest in country $c$ at date $t$, and $\varepsilon_{ct}$ is an idiosyncratic error term.

The identification assumption we rely on for our estimates to be interpreted as causal is that conditional on country-year, month-of-the-year, and day-of-the-week fixed effects, the within-year date of disclosure of a scandal is as-good-as-randomly assigned in relation to the protest baseline of the country. The country-year fixed effects absorb every source of country-year-level confounding such as pre-existing political instability, electoral calendars, or institutional reforms. The day-of-the-week and month-of-the-year fixed effects absorb weekly and seasonal cycles in protest activity\footnote{In \autoref{sup:random} we show that, \emph{within} a country-year, the exact date of a scandal's disclosure cannot be predicted from the day of the week, the month, the day of the month, or proximity to a national election: the out-of-sample within-country-year AUC is $0.54$---essentially a coin flip (AUC=$0.5$).}.
...
\autoref{tab:main} reports OLS estimates of the post-scandal effect on violent and peaceful protests, depending on the political rank of the implicated official. We show the estimates on each outcome for 30-, 60-, and 90-day windows\footnote{In \autoref{sup:window} we show our estimates are robust to window widths of 15- to 150-day windows, even though, as expected, effects decay on wider windows.}. The contrast is stark. Scandals implicating Apex officials increase the daily count of violent protests by 0.051 (P$=0.007$), 0.040 (P$=0.004$), or 0.026 (P$=0.018$) in the $\pm30$-, $60$-, and $90$-day windows respectively. The estimates are large, as they represent 2.6, 1.9, or 1.1 times the pre-scandal means in each of the three window-widths. Scandals implicating Non-Apex officials, on the other hand, do not cause changes in violent protests, with corresponding estimates of a 0.006 decrease in the 30-day window, and 0.003 and 0.002 increases in the 60- and 90-day window respectively, all of them statistically insignificant. Importantly, our estimates show little to no response of peaceful protests to scandals implicating either type of official, with the largest estimate being a 0.019 increase (P$=0.144$) in the daily count of peaceful protests in a 30-day window of Non-Apex scandals. 
...
To trace the dynamics of the effect, we replace the post-scandal indicators with a fully saturated set of 15-day time-to-event bin indicators over the $\pm 60$-day window; the reference bin is the bin immediately before the scandal disclosure (i.e., $w_{s,t} \in [-15, -1]$). Furthermore, to tighten up the comparisons we estimate \autoref{eq:main} separately on the Apex and Non-Apex subsamples\footnote{Note that in \autoref{eq:main} we are comparing the average number of protest occurrences after an Apex/Non-Apex scandal to the average number of protest occurrences before the scandal, pooled across both types of scandals. Thus, if a country has an Apex and Non-Apex scandal with overlapping windows, the comparison count of protests might be an incorrect counterfactual.}.

\autoref{fig:dynamics} traces the dynamics of the effect. In the Apex subsample, violent protests (\autoref{fig:dynamics:pav}) show arguably no pre-trend and then jump sharply at disclosure, remaining elevated even in the 60 days that follow; peaceful protests (\autoref{fig:dynamics:pap}) stay flat at pre-scandal levels in the full 60 days following apex scandal disclosures. In the Non-Apex subsample, violent protests (\autoref{fig:dynamics:nav}) remain flat throughout, showing no changes relative to the pre-scandal baseline, and changes in peaceful protests (\autoref{fig:dynamics:nap}) patterns are also statistically indistinguishable from zero. Our estimates show that, if anything, there is somewhat of a \textit{decrease} in peaceful protest occurrences. The absence of any pre-scandal disclosure trends in protest occurrences for both types of scandals and outcomes supports our identifying assumption. If scandal disclosure dates were predictable by citizens, or citizens found out about scandals before they were disclosed by the media, we would expect to see increases in violent protests in the days leading up to scandal disclosures. We do not observe this pattern\footnote{In \autoref{sup:rd} we show that our results are robust to allowing for pre-scandal disclosure trends in a regression discontinuity in time design, where we use the days from/to scandal disclosures as the running variable.}.
...
To test these hypotheses, we classify the countries in our sample into High/Medium/Low Democracy by their 2008 level of democracy\footnote{We pick 2008 since this is the year in which our sample of scandals starts. Picking later years for the classification would make our democracy measure endogenous to our object of interest.}, as measured by V-Dem's Electoral Democracy Index~\citep{vdem2024}. In particular we classify countries in the bottom tercile of the index's distribution as Low Democracy, those above the bottom tercile but below the top tercile as Medium Democracy, and those in the top tercile as High Democracy\footnote{See \autoref{sup:democracy_alt} for the classification of the countries in our sample.}.

In \autoref{tab:sup_democracy_lmh} we show estimates of an equation analogous to \autoref{eq:main}, but substituting the Apex and Non-Apex indicators for indicators of the country in which the scandal occurs being Low vs. Medium or High Democracy in 2008. Contrary to what we expected, the effects are larger---and remarkably consistent across time windows---in countries with higher levels of democracy. In the 30-day window, corruption scandals cause an increase of 0.016 (P$=0.022$) violent protests in countries with higher democratic levels, whereas they only cause a 0.012 (P$=0.046$) increase in the countries with lower democratic levels, though we cannot reject the one-sided hypothesis for the effect being larger than or equal in countries with lower levels of democracy. A similar story is apparent in the 60-day window, where corruption scandals cause an increase of 0.021 (P$=0.020$) in the daily count of violent protests in more democratic countries, and just a 0.010 (P$=0.096$) increase in less democratic ones. Lastly, we estimate an increase of 0.019 (P$=0.075$) daily violent protests in more democratic countries, and a corresponding insignificant 0.001 (P$=0.947$) increase in countries with lower levels of democracy\footnote{We do not find evidence that these patterns can be explained by violent responses of the government towards protestors. Results available upon request.}.
...
\begin{table}[htb!]
    ...
    \end{center}
    \footnotesize{This table shows estimates of the effect of apex corruption scandal disclosures on violent and peaceful protests by level of democracy in 2008 in the country where the scandal occurs. The count of violent protests at the country-day level is the outcome in odd-numbered columns, while the count of peaceful protests at the country-day level is the outcome in even-numbered columns. The first two columns use a $\pm30$-day window for the analysis, the next two columns use a $\pm60$-day window for the analysis, and the last two columns use a $\pm90$-day window for the analysis. ``$p$-value: Medium+High $=$ Low'' reports the two-sided $p$-value for $H_0:\beta_{\text{Medium+High}}=\beta_{\text{Low}}$; ``$p$-value: Medium+High $>$ Low (one-sided)'' reports the one-sided $p$-value. An observation is a country-day. All specifications include country-year, month-of-the-year, and day-of-the-week fixed effects; standard errors in parentheses are clustered at the country $\times$ year $\times$ day-bin level. Sample period 2008--2019; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}
...
% =====================================================================
\section{Discussion}
The evidence we have presented suggests citizens do not respond uniformly to all types of corruption: they respond sharply, immediately and durably when the implicated official is at the apex of the political spectrum, and they barely respond when the implicated official sits at the lower tier of the hierarchy. Furthermore, the response concentrates on violent protests and persists even in a 60-day window. The peaceful margin stays essentially unchanged over all three horizons and regardless of whether the scandal implicates an Apex or Non-Apex official. Our findings underscore the fact that corruption is a tangible behavioral shock to the local (and perhaps even regional) political environment, not just an attitudinal one. The stakes citizens hold in the continuation in office of a corrupt official appear to govern whether citizens mobilize, and how they do so. The joint pattern of a persistent rise in violent protests with no corresponding change on the peaceful margin is consistent with, though our design does not establish, citizens turning to violent rather than peaceful channels when a scandal undercuts their confidence in the officials meant to be held accountable by democratic institutions.

Our findings contribute to three literatures. First, the economics-of-conflict literature has documented that political violence responds to economic shocks~\citep{besley2011logic,berman2011hearts,dube2013commodity,koenig2017networks,mcguirk2020economic,rohner2026aidingpeace}. We introduce a \textit{political} shock---public disclosure of apex corruption---and show that it produces mobilization responses of sizable magnitudes. Second, the corruption and accountability literature has documented that disclosures of malfeasance change voting behavior~\citep{ferraz2008exposing,ferraz2011electoral,FerrazFinanAccount,ferraz2018audits,horacio2019infonetwork}. The slow electoral horizon has tended to obscure that the same disclosures can have very fast, non-electoral consequences~\citep{madestam2013protests,bursztyn2021persistent}. Our high-frequency design uncovers a channel through which apex corruption shapes the political environment in democracies within days of public scandal disclosures, well before subsequent elections come into play. Third, we show that the protest channel is a behavioral complement of the attitudinal evidence from our companion paper~\citep{ApexCorruption}, which documents that apex corruption scandals erode support for democracy and democratic institutions, and also reduce political participation.

Our results help reconcile part of the mixed results in the existing literature on the political consequences of corruption disclosures. Studies of local level corruption have produced muted estimates of disclosure's electoral consequences, with some finding modest accountability gains in Brazil and Spain~\citep{ferraz2008exposing,ferraz2011electoral,costasperez2012corruption,ferraz2018audits} and others finding null or even demobilizing effects~\citep{anduiza2013turning,chong2015does,pavao2018corruption,horacio2022metaketa}. We argue that the dispersion in municipal estimates reflects the limited political stakes at that level. In particular, when the implicated official is one of many municipal officeholders, citizens have weaker incentives to incur the costs of mobilization. Within our sample, scandals implicating Congressmen, lower judiciary and lower rank officials have immediate (30-day) post-scandal effects on violent protests statistically indistinguishable from zero, whereas the effect of scandals implicating Apex officials is large in a comparable time horizon.

A ``salience of stakeholders'' view rationalizes these observed patterns~\citep{acemoglu2006origins,acemoglu2012nations,jha2013trade,jha2019valuing,jha2022markets}. Citizens' political behavior responds to the political weight of \textit{what} is being disclosed. Apex corruption, thus, affects citizens' perceived stakes of the political bargain, particularly when the implicated official is at the apex of the political hierarchy. The contrast between high--stakes apex scandals and low-stakes non-apex scandals parallels the contrast between high-stakes elections and routine governance in studies of political accountability~\citep{martinezbravo2022elites}. Notably, weak institutions do not seem to be a necessary condition for this response to appear, as the response is present across our sample and, if anything, somewhat stronger in the more democratic countries, where citizens plausibly hold higher expectations of accountability---and enjoy greater latitude to voice them---when those expectations are violated~\citep{acemoglu2019DemoEcoGrowth,acemoglu2024demobreedsupport,DuranteFutbol}.
...
\begin{center}
    {\Large\bfseries Supplementary Materials for \\ ``\papertitle'' \\ \small{Roberto Gonz\'alez-T\'ellez, Saumitra Jha, Eduardo Rivera and Enrique Seira}}
\end{center}
\bigskip

\section{Some protests in our data}\label{sup:protests_examples}

The Mass Mobilization (MM) project~\citep{clark2016mass} records, for each protest, its start date, its location, a coarse estimate of the number of participants, the protesters' demands, the state's response, and---central to our analysis---whether the protest was \emph{violent} (``[t]he violence could include anything from riotous behavior that destroys property to shooting at the police or military''~\citep{clark2016mass}). To convey what these records look like and how they line up with our scandals, we describe four protests that the MM data place within a few days of a scandal's disclosure: two that follow an \emph{apex} scandal and two that fall near a \emph{non-apex} scandal, with one violent and one peaceful episode in each pair.

\paragraph{Brazil, May 2017 --- Apex scandal --- Violent protest.} On May 17, 2017 Brazilian media released a covert recording in which an executive of the meat-packer JBS appears to capture President Michel Temer endorsing hush-money payments to a jailed former speaker of the lower house. The next day the MM data record a national, \emph{violent} protest of roughly 35,000 participants whose coded demand is ``political behavior, process'' and whose coded state response is ``crowd dispersal'' and ``arrests''; MM's own notes describe crowds calling on Temer to resign ``after a newspaper reported that prosecutors have a tape of him agreeing to the payment of hush money,'' a demonstration that ``deteriorated into violence'' with tear gas and rubber bullets and, within days, a deployment of federal troops in Bras\'ilia. The protest names the scandal as its grievance.

\paragraph{Guatemala, August 2015 --- Apex scandal --- Peaceful protest.} On August 24, 2015 prosecutors and the UN-backed CICIG named President Otto P\'erez Molina as the leader of the ``La L\'inea'' customs-fraud ring. Three days later the MM data record a \emph{peaceful} mass mobilization in Guatemala City, on the order of tens of thousands, whose coded demand is the ``removal of politician''; MM's notes read ``tens of thousands of protesters in Guatemala City call on President Otto Perez to step down,'' and the coded state response is ``ignore.'' The president lost his immunity on September 1 and resigned on September 3, 2015.

\paragraph{Colombia, January 2017 --- Non-Apex scandal --- Violent protest.} In mid-January 2017 Gabriel Garc\'ia Morales---a former deputy transport minister and head of the national road-concessions agency---became the first Colombian charged in the Odebrecht affair, for taking a USD~6.5~million bribe to steer the Ruta del Sol highway concession to the firm. Nine days later the MM data record a \emph{violent} protest in Bogot\'a, but on an entirely unrelated matter: animal-rights demonstrators clashed with riot police---hurling bottles and stones and met with tear gas---over the return of bullfighting to the city's bullring. The episode is coded violent, with a demand of ``political behavior, process,'' yet it has nothing to do with the corruption charge whose event window it falls in.

\paragraph{Brazil, November 2012 --- Non-Apex scandal --- Peaceful protest.} On November 24, 2012, following the Federal Police's Operation Porto Seguro, President Dilma Rousseff dismissed a set of mid-level officials---the head of the presidency's regional office in S\~ao Paulo and the directors of two federal agencies---for running a network that falsified technical reports. The MM data record a large, peaceful protest two days later (Rio de Janeiro, on the order of tens of thousands), but its subject is unrelated to the dismissals: MM's notes describe a crowd ``protesting over oil revenue sharing proposals,'' i.e.\ a bill on how oil royalties are split across states. Proximity in time does not necessarily make the protest about the scandal.

\clearpage
\section{Scandal classification}\label{sup:classification}

We classify each of the 174 scandals in our panel by the position of the implicated official. Using the raw text of the news article reporting on the scandal, we assign a code to the scandal: \texttt{president} for current or former presidents, \texttt{governor} for current or former governors, \texttt{supreme court justice} for Supreme Court Justices, \texttt{congressman} for national legislators, \texttt{other judiciary} for lower-court judges and prosecutors, and \texttt{others} for ministers, agency heads, municipal mayors, and all remaining officials not included in one of the former categories. We then classify scandals in the \texttt{president}, \texttt{governor}, or \texttt{supreme court justice} categories to the Apex class, and the remaining ones to the Non-Apex class. \autoref{tab:sup_scandal_list} shows the country, year, description and class associated to each of our 174 scandals. 

\input{sup_scandal_list}

% ---------------------------------------------------------------------
\clearpage
\section{The timing of scandal disclosures is unpredictable}\label{sup:random}

Our identification rests on the exact calendar date of a scandal's disclosure being as-good-as-randomly distributed within a country-year. We ask whether the day a scandal is disclosed can be told apart from the other days of the \emph{same} country-year using calendar structure and the electoral cycle. We estimate a conditional---on country-year fixed effects---logit and measure discrimination with the within-country-year AUC. That is, the probability the model ranks the true scandal-day above a random non-scandal-day of the same country-year.

\begin{table}[htb!]
    \caption{Scandal disclosure dates are as-good-as-random}
    \label{tab:scandal_random}
    \begin{center}\input{scandals_as_random}\end{center}
    \footnotesize{This table shows how well the exact disclosure date of a corruption scandal can be predicted within a country-year, using progressively richer sets of predictors. Each column is a conditional---country-year fixed effects---logit of an indicator for a scandal being disclosed on a given country-day, on the day-of-the-week, the month-of-the-year, the day-of-the-month, and proximity to a national election (indicators for a day falling 0--30, 31--60, 61--90, or 91--120 days before an election), with each column adding one predictor set to the previous. An observation is a country-day; conditioning on country-year restricts the comparison to the 79 country-years that contain a scandal (28{,}852 country-days). ``Joint test $p$-value'' is the Wald test that the predictors added in that column are jointly different from 0. ``Within-year AUC'' is the probability the model ranks the true scandal-day above a random non-scandal-day of the same country-year, where $0.5$ is a coin flip---and the out-of-sample row trains on a random half of the country-days and evaluates on the held-out half. Sample period 2008--2019; Venezuela excluded.}
\end{table}
...
We re-estimate the OLS specification at event-window widths $T \in \{15, 30, 45, \dots, 150\}$ days and plot the point estimate against $T$ with a 90\% confidence interval. \autoref{fig:sup_window} places the Apex and Non-Apex subsamples \emph{side by side}---violent protests in the top row, peaceful in the bottom---on a common y-axis across all four panels. The $T=30, 60, 90$ window-widths reported in \autoref{tab:uniquetocorruption} are shown with diamond markers.
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{This figure shows estimates of our OLS specification for each window-width $T \in \{15, 30, \dots, 150\}$. Vertical bars are 90\% confidence intervals; the estimates at $T=30, 60, 90$ (the widths in \autoref{tab:uniquetocorruption}) are shown with red diamonds. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
...
In this section we re-analyze our sample with a discontinuity-in-time design, using the number of days from/to a given scandal as the running variable and setting the disclosure date as the cutoff value. We allow the polynomial on each side of the cutoff to differ in shape but not in degree. We partial-out month-of-the-year, day-of-the-week and country-year fixed effects.

\autoref{tab:sup_rd_time} reports the estimated immediate (at the disclosure date) effect for the Apex and Non-Apex subsamples, on violent and peaceful protests, for bandwidths $h\in\{30,60,90\}$ days and local-linear, local-quadratic polynomials, and a non-parametric fit.

\begin{table}[htb!]
    \caption{Regression discontinuity in event time: jump at disclosure.}
    \label{tab:sup_rd_time}
    \begin{center}
        \scriptsize{\input{sup_rd_time}}
    \end{center}
    \footnotesize{This table shows estimates of $\tau$ in $Y_{c(s)t} = \tau\,\text{Post}_{st} + f(w_{s,t}) + \text{Post}_{st}\cdot g(w_{s,t}) + \alpha_d + \lambda_m + \theta_{cy} + \varepsilon_{c(s)t}$, where the polynomials $f,g$ are local-linear or local-quadratic in the running variable, the running variable being centred at $0$. The ``Nonparametric'' rows report estimates using a local-polynomial with MSE-optimal data-driven bandwidth and a triangular kernel, computed within the $\pm h$ window on the partialed-out outcome. We estimate each effect separately on each of the Apex and Non-Apex subsamples, on violent and peaceful protest counts, and at windows $h\in\{30,60,90\}$ days. All specifications include country-year, month-of-the-year, and day-of-the-week fixed effects; standard errors in parentheses clustered at the country $\times$ year $\times$ day-bin level. Sample period 2008--2019; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}
...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{This figure shows 5-day binned means of the violent protest counts, partialled-out on country-year, month-of-the-year, and day-of-the-week fixed effects, with local-linear fits on each side of the disclosure cutoff at the $h=30-, 60-, 90$-day horizons. Subfigures on the left-hand side show estimates on the Apex subsample, and subfigures on the right-hand side show estimates on the Non-Apex subsample. Sample period 2008--2019; Venezuela excluded.}
\end{figure}

\begin{figure}[htb!]
    \caption{Regression discontinuity in event time, peaceful protests, at $h=30/60/90$-day bandwidths.}
    \label{fig:sup_rd_time_peaceful}
    \begin{center}
    \begin{subfigure}{0.45\textwidth}\caption{Apex, $h=30$.}
        \includegraphics[width=\textwidth]{rd_time_num_peaceful_MM_pa_h30.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $h=30$.}
        \includegraphics[width=\textwidth]{rd_time_num_peaceful_MM_na_h30.pdf}\end{subfigure}

    \begin{subfigure}{0.45\textwidth}\caption{Apex, $h=60$.}
        \includegraphics[width=\textwidth]{rd_time_num_peaceful_MM_pa_h60.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $h=60$.}
        \includegraphics[width=\textwidth]{rd_time_num_peaceful_MM_na_h60.pdf}\end{subfigure}

    \begin{subfigure}{0.45\textwidth}\caption{Apex, $h=90$.}
        \includegraphics[width=\textwidth]{rd_time_num_peaceful_MM_pa_h90.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $h=90$.}
        \includegraphics[width=\textwidth]{rd_time_num_peaceful_MM_na_h90.pdf}\end{subfigure}
    \end{center}
    \footnotesize{This figure shows 5-day binned means of the peaceful protest counts, partialled-out on country-year, month-of-the-year, and day-of-the-week fixed effects, with local-linear fits on each side of the disclosure cutoff at the $h=30-, 60-, 90$-day horizons. Subfigures on the left-hand side show estimates on the Apex subsample, and subfigures on the right-hand side show estimates on the Non-Apex subsample. Common vertical scale across all regression-discontinuity figures. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{This figure shows 5-day binned means of the violent protest counts, partialled-out on country-year, month-of-the-year, and day-of-the-week fixed effects, with local-quadratic fits on each side of the disclosure cutoff at the $h=30-, 60-, 90$-day horizons. Subfigures on the left-hand side show estimates on the Apex subsample, and subfigures on the right-hand side show estimates on the Non-Apex subsample. Common vertical scale across all regression-discontinuity figures. Sample period 2008--2019; Venezuela excluded.}
\end{figure}

\begin{figure}[htb!]
    \caption{Regression discontinuity in event time, local-quadratic fit, peaceful protests, at $h=30/60/90$-day bandwidths.}
    \label{fig:sup_rd_time_quad_peaceful}
    \begin{center}
    \begin{subfigure}{0.45\textwidth}\caption{Apex, $h=30$.}
        \includegraphics[width=\textwidth]{rd_time_quad_num_peaceful_MM_pa_h30.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $h=30$.}
        \includegraphics[width=\textwidth]{rd_time_quad_num_peaceful_MM_na_h30.pdf}\end{subfigure}

    \begin{subfigure}{0.45\textwidth}\caption{Apex, $h=60$.}
        \includegraphics[width=\textwidth]{rd_time_quad_num_peaceful_MM_pa_h60.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $h=60$.}
        \includegraphics[width=\textwidth]{rd_time_quad_num_peaceful_MM_na_h60.pdf}\end{subfigure}

    \begin{subfigure}{0.45\textwidth}\caption{Apex, $h=90$.}
        \includegraphics[width=\textwidth]{rd_time_quad_num_peaceful_MM_pa_h90.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $h=90$.}
        \includegraphics[width=\textwidth]{rd_time_quad_num_peaceful_MM_na_h90.pdf}\end{subfigure}
    \end{center}
    \footnotesize{This figure shows 5-day binned means of the peaceful protest counts, partialled-out on country-year, month-of-the-year, and day-of-the-week fixed effects, with local-quadratic fits on each side of the disclosure cutoff at the $h=30-, 60-, 90$-day horizons. Subfigures on the left-hand side show estimates on the Apex subsample, and subfigures on the right-hand side show estimates on the Non-Apex subsample. Common vertical scale across all regression-discontinuity figures. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{This figure shows binned means (12 per side) of the violent protest counts, partialled-out on country-year, month-of-the-year, and day-of-the-week fixed effects, with non-parametric local-linear fits (triangular kernel, MSE-optimal data-driven bandwidth) and 90\% confidence bands on each side of the disclosure cutoff at the $\pm30-, \pm60-, \pm90$-day windows. Subfigures on the left-hand side show estimates on the Apex subsample, and subfigures on the right-hand side show estimates on the Non-Apex subsample; the legend in each reports the discontinuity estimate $\tau$ and its $p$-value. Common vertical scale across all regression-discontinuity figures. Sample period 2008--2019; Venezuela excluded.}
\end{figure}

\begin{figure}[htb!]
    \caption{Regression discontinuity in event time, non-parametric fit, peaceful protests, at $\pm30/60/90$-day windows.}
    \label{fig:sup_rd_time_np_peaceful}
    \begin{center}
    \begin{subfigure}{0.45\textwidth}\caption{Apex, $\pm30$.}
        \includegraphics[width=\textwidth]{rd_time_np_num_peaceful_MM_pa_h30.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $\pm30$.}
        \includegraphics[width=\textwidth]{rd_time_np_num_peaceful_MM_na_h30.pdf}\end{subfigure}

    \begin{subfigure}{0.45\textwidth}\caption{Apex, $\pm60$.}
        \includegraphics[width=\textwidth]{rd_time_np_num_peaceful_MM_pa_h60.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $\pm60$.}
        \includegraphics[width=\textwidth]{rd_time_np_num_peaceful_MM_na_h60.pdf}\end{subfigure}

    \begin{subfigure}{0.45\textwidth}\caption{Apex, $\pm90$.}
        \includegraphics[width=\textwidth]{rd_time_np_num_peaceful_MM_pa_h90.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $\pm90$.}
        \includegraphics[width=\textwidth]{rd_time_np_num_peaceful_MM_na_h90.pdf}\end{subfigure}
    \end{center}
    \footnotesize{This figure shows binned means (12 per side) of the peaceful protest counts, partialled-out on country-year, month-of-the-year, and day-of-the-week fixed effects, with non-parametric local-linear fits (triangular kernel, MSE-optimal data-driven bandwidth) and 90\% confidence bands on each side of the disclosure cutoff at the $\pm30-, \pm60-, \pm90$-day windows. Subfigures on the left-hand side show estimates on the Apex subsample, and subfigures on the right-hand side show estimates on the Non-Apex subsample; the legend in each reports the discontinuity estimate $\tau$ and its $p$-value. Common vertical scale across all regression-discontinuity figures. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{This figure shows randomization inference for violent protests. Each histogram is the distribution of 1{,}000 placebo estimates of the post-scandal effect, obtained by redrawing each scandal's disclosure date at random within its own country-year and re-estimating the OLS specification on the $\pm T$ event window. Top row uses the Apex subsample. The bottom row uses the Non-Apex subsample. Columns are the $\pm 30$, $\pm 60$, and $\pm 90$ day windows. The maroon vertical line (drawn on top) marks the observed $\hat\beta$ from the corresponding cell of \autoref{tab:uniquetocorruption}; the top-right legend reports that observed $\hat\beta$ and the two-sided randomization-inference $p$-value. Standard errors clustered at country $\times$ year $\times$ day-bin. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{This figure shows randomization inference for peaceful protests. Each histogram is the distribution of 1{,}000 placebo estimates of the post-scandal effect, obtained by redrawing each scandal's disclosure date at random within its own country-year and re-estimating the OLS specification on the $\pm T$ event window. Top row uses the Apex subsample. The bottom row uses the Non-Apex subsample. Columns are the $\pm 30$, $\pm 60$, and $\pm 90$ day windows. The maroon vertical line (drawn on top) marks the observed $\hat\beta$ from the corresponding cell of \autoref{tab:uniquetocorruption}; the top-right legend reports that observed $\hat\beta$ and the two-sided randomization-inference $p$-value. Standard errors clustered at country $\times$ year $\times$ day-bin. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
...
\autoref{tab:sup_democracy_index_ranks} lists the countries in our sample, sorted by their 2008 V-Dem Electoral Democracy Index (the index used in \autoref{tab:sup_democracy_lmh} in the main text), and shows how each is classified under four different 2008 democracy measures. \autoref{tab:sup_democracy_terciles_libdem_lmh} then re-estimates the main-text Low vs.\ Medium+High split with V-Dem's Liberal Democracy Index, and \autoref{tab:sup_democracy_fh} with Freedom House's 2008 status (Free vs.\ Partly Free) in place of a continuous index. 

\begin{table}[htb!]
    \caption{Country democracy classification across four indices.}
    \label{tab:sup_democracy_index_ranks}
    \begin{center}
        \small{\input{sup_democracy_index_ranks}}
    \end{center}
    \footnotesize{This table shows each country's classification under four 2008 democracy measures. For the three continuous indices---V-Dem Electoral Democracy, V-Dem Liberal Democracy, and Polity2---the entry is the country's tercile (High / Medium / Low). Freedom House status is shown in its native categories (Free / Partly Free / Not Free). Polity2 discriminates poorly among the democracies in our sample, so its tercile split is noisier than the V-Dem indices. Countries are sorted by the V-Dem Electoral index.}
\end{table}
...
\begin{table}[htb!]
    ...
    \begin{center}
        \scriptsize{\input{sup_democracy_terciles_libdem_lmh_ols}}
    \end{center}
    \footnotesize{This table shows estimates of the effect of apex corruption scandal disclosures on violent and peaceful protests by level of democracy in 2008 in the country where the scandal occurs, classifying countries by their 2008 value of V-Dem's Liberal Democracy Index (\texttt{v2x\_libdem}) rather than the Electoral Democracy Index used in \autoref{tab:sup_democracy_lmh}. The count of violent protests at the country-day level is the outcome in odd-numbered columns, while the count of peaceful protests at the country-day level is the outcome in even-numbered columns. The first two columns use a $\pm30$-day window for the analysis, the next two columns use a $\pm60$-day window for the analysis, and the last two columns use a $\pm90$-day window for the analysis. ``$p$-value: Medium+High $=$ Low'' reports the two-sided $p$-value for $H_0:\beta_{\text{Medium+High}}=\beta_{\text{Low}}$; ``$p$-value: Medium+High $>$ Low (one-sided)'' reports the one-sided $p$-value. An observation is a country-day. All specifications include country-year, month-of-the-year, and day-of-the-week fixed effects; standard errors in parentheses are clustered at the country $\times$ year $\times$ day-bin level. Sample period 2008--2019; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}
...
\begin{table}[htb!]
    ...
    \begin{center}
        \scriptsize{\input{sup_democracy_fh_ols}}
    \end{center}
    \footnotesize{This table shows estimates of the effect of apex corruption scandal disclosures on violent and peaceful protests by level of democracy in 2008 in the country where the scandal occurs, classifying countries by their 2008 Freedom House status (\texttt{e\_fh\_status}) rather than a V-Dem index; among the 16 sample countries none is rated Not Free, so the comparison is Free (9 countries) versus Partly Free (7 countries). The count of violent protests at the country-day level is the outcome in odd-numbered columns, while the count of peaceful protests at the country-day level is the outcome in even-numbered columns. The first two columns use a $\pm30$-day window for the analysis, the next two columns use a $\pm60$-day window for the analysis, and the last two columns use a $\pm90$-day window for the analysis. ``$p$-value: Free $=$ Partly Free'' reports the two-sided $p$-value for $H_0:\beta_{\text{Free}}=\beta_{\text{Partly Free}}$; ``$p$-value: Free $>$ Partly Free (one-sided)'' reports the one-sided $p$-value. An observation is a country-day. All specifications include country-year, month-of-the-year, and day-of-the-week fixed effects; standard errors in parentheses are clustered at the country $\times$ year $\times$ day-bin level. Sample period 2008--2019; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}
...
\begin{table}[htb!]
    ...
    \begin{center}\scriptsize
        ...
        \input{did_ra_main_pa}\\[10pt]
        \textbf{Non-Apex sample}\\[3pt]
        \input{did_ra_main_na}
    \end{center}
...
We re-estimate \autoref{eq:main} on violent and peaceful protests in the $\pm$30-day window, dropping one scandal's event window at a time.

\begin{figure}[htb!]
    \caption{Leave-one-out by scandal type on violent protests.}
    \label{fig:sup_loo}
    \begin{center}
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{This figure shows leave-one-out estimates of the two-group interaction coefficients (\autoref{tab:main}) on violent protests, at the $\pm 30$-day window. Each point re-estimates the panel's coefficient---Post $\times$ Apex in (a), Post $\times$ Non-Apex in (b)---dropping the labelled scandal's event window; panel (a) leaves out each of the 63 apex scandals, panel (b) each of the 111 non-apex scandals. Vertical bars are 90\% confidence intervals; scandals (x-axis, labelled Country -- Year, with a trailing ordinal ordering scandals by date within a country-year when there is more than one, e.g.\ ``Peru -- 2017 -- 3'') are sorted by the estimate. The red horizontal line is the coefficient on all scandals and the faint grey line marks zero; the two panels share a common vertical scale. Standard errors clustered at country $\times$ year $\times$ day-bin. Sample period 2008--2019; Venezuela excluded.}
\end{figure}

\begin{figure}[htb!]
    \caption{Leave-one-out by scandal type on peaceful protests.}
    \label{fig:sup_loo_peaceful}
    \begin{center}
    \begin{subfigure}{\textwidth}
        \caption{Apex: $\beta_{\text{Apex}}$ (Post $\times$ Apex), dropping each apex scandal.}
        \includegraphics[width=\textwidth]{loo_scandal_beta_apex_peaceful_w30.pdf}
    \end{subfigure} \\[6pt]
    \begin{subfigure}{\textwidth}
        \caption{Non-Apex: $\beta_{\text{Non-Apex}}$ (Post $\times$ Non-Apex), dropping each non-apex scandal.}
        \includegraphics[width=\textwidth]{loo_scandal_beta_nonapex_peaceful_w30.pdf}
    \end{subfigure}
    \end{center}
    \footnotesize{This figure shows leave-one-out estimates of the two-group interaction coefficients (\autoref{tab:main}) on peaceful protests, at the $\pm 30$-day window. Each point re-estimates the panel's coefficient---Post $\times$ Apex in (a), Post $\times$ Non-Apex in (b)---dropping the labelled scandal's event window; panel (a) leaves out each of the 63 apex scandals, panel (b) each of the 111 non-apex scandals. Vertical bars are 90\% confidence intervals; scandals (x-axis, labelled Country -- Year, with a trailing ordinal ordering scandals by date within a country-year when there is more than one, e.g.\ ``Peru -- 2017 -- 3'') are sorted by the estimate. The red horizontal line is the coefficient on all scandals and the faint grey line marks zero; the two panels share a common vertical scale. Standard errors clustered at country $\times$ year $\times$ day-bin. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
...
% ---------------------------------------------------------------------
\clearpage
\section{Protest grievance: corruption-related vs.\ unrelated}\label{sup:grievance}

Because our outcome counts \emph{all} violent protests at the country-day level, a natural question is whether the apex effect operates through protests \emph{about} the scandal or through a broader wave of mobilization whose grievances need not concern corruption (\autoref{sup:protests_examples} gives examples of both). To probe this, we classify each Mass Mobilization protest as \emph{corruption/accountability-related} or \emph{unrelated} using its coded demand and its free-text description: a protest is corruption-related if the MM \texttt{notes} mention corruption or malfeasance (e.g.\ ``corruption'', ``bribe'', ``embezzlement'', ``Odebrecht'', ``Lava Jato''), if its coded demand is ``removal of politician'', or if the notes call for a resignation or impeachment; all other protests are coded unrelated. About a quarter of the protests in our sample-country panel are corruption-related by this rule. We rebuild the daily counts separately for the two grievance classes---which by construction sum to the totals used throughout the paper---and re-estimate \autoref{eq:main} on each.

\begin{table}[htb!]
    \caption{Effect on violent and peaceful protests, split by whether the protest's grievance is corruption-related.}
    \label{tab:grievance_split}
    \begin{center}
        \scriptsize{\input{grievance_split}}
    \end{center}
    \footnotesize{This table re-estimates \autoref{eq:main} separately on daily counts of corruption/accountability-related and unrelated protests. A protest is corruption-related if its Mass Mobilization \texttt{notes} mention corruption or malfeasance, if its coded demand is ``removal of politician'', or if the notes call for a resignation or impeachment; otherwise it is unrelated. Columns 1--2 are violent protests (corruption-related, then unrelated); columns 3--4 are peaceful protests. Each panel uses a $\pm h$-day window. Two-sided $p$-values in brackets, computed from standard errors clustered at the country $\times$ year $\times$ day-bin level. All specifications include country-year, month-of-the-year, and day-of-the-week fixed effects. The corruption-related and unrelated counts sum to the totals in \autoref{tab:main}. Sample period 2008--2019; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}

\autoref{tab:grievance_split} shows two patterns. First, the apex effect on violent protests is present in the corruption-related count: for apex scandals the daily count of corruption-related violent protests rises by $0.014$ (p$=0.029$) in the 30-day window and $0.011$ (p$=0.016$) in the 60-day window, while the non-apex estimates are indistinguishable from zero. Second, a comparable---indeed somewhat larger---share of the apex effect appears in protests our classifier labels unrelated to corruption ($0.037$, $0.029$, and $0.023$ at the 30-, 60-, and 90-day windows, each significant), again with no response for non-apex scandals. Across windows the two components sum to the headline apex violent-protest effect ($0.014+0.037\approx0.051$ at 30 days, $0.011+0.029=0.040$ at 60 days, $0.003+0.023=0.026$ at 90 days). The peaceful margin does not respond to apex scandals in either grievance class.

We read this as follows. The response remains squarely \emph{apex-specific}: non-apex scandals move neither the corruption-related nor the unrelated violent-protest count, in any window. What the split adds is that an apex-corruption disclosure is followed not only by protests explicitly framed around the scandal but by a broader rise in violent mobilization, consistent with apex corruption acting as a shock to the political environment rather than triggering narrowly anti-corruption protest alone. Two caveats temper the corruption-vs-unrelated contrast. First, the MM descriptions are terse, so a protest that is part of the scandal-driven wave but not explicitly framed as anti-corruption is coded ``unrelated''; the classifier therefore likely \emph{understates} the corruption-related share, which should be read as a lower bound on the scandal-attributable response. Second, the assignment is keyword-based and inherently coarse. The apex/non-apex contrast---robust across both columns and both margins---remains the more reliable comparison.

% =====================================================================
% GRIEVANCE-SPLIT REPLICATION OF THE MAIN EVIDENCE
% =====================================================================
\clearpage
\section{Grievance-split replication of the main evidence}\label{sup:grievance_repl}

Building on the grievance classification of \autoref{sup:grievance}, this section reproduces each of the paper's main pieces of evidence separately for corruption/accountability-related and unrelated protests, to check whether the apex-vs.-non-apex pattern persists within each grievance class. For every \emph{table} we add grievance panels while keeping the original column layout; for every \emph{figure} we overlay the two classes---corruption-related in a solid black line with circle markers, unrelated in a short-dashed grey line with square markers. The verdict is that the paper's structure survives the split: the rise in violent protest after apex scandals, and its absence after non-apex scandals, holds within the corruption-related count and---somewhat more strongly---within the unrelated count, while the peaceful margin stays flat in both. Apex corruption is followed by a broad wave of violent mobilization that includes, but is not limited to, protests explicitly about the scandal.

\subsection{Main interaction}

\begin{table}[htb!]
    \caption{\autoref{tab:main} split by protest grievance.}
    \label{tab:g_interaction}
    \begin{center}
        \scriptsize{\input{g_interaction_panels}}
    \end{center}
    \footnotesize{This table re-estimates \autoref{eq:main} (the two-group interaction of \autoref{tab:main}) separately on corruption-related protests (Panel A) and unrelated protests (Panel B). Columns are violent/peaceful protest counts at the $\pm30/60/90$-day windows. Standard errors (in parentheses) clustered at the country $\times$ year $\times$ day-bin level; all specifications include country-year, month-of-the-year, and day-of-the-week fixed effects. Sample period 2008--2019; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}

\autoref{tab:g_interaction} is the panelled counterpart of the summary in \autoref{tab:grievance_split}. In both grievance panels the daily count of violent protests rises after \emph{apex} scandals and is flat after non-apex ones: the Post $\times$ Apex coefficient on violent protests is $0.014$ (corruption-related) and $0.036$ (unrelated) in the 30-day window, each significant, with non-apex coefficients indistinguishable from zero. The peaceful margin does not respond in either panel.

\subsection{Protest dynamics}

\begin{figure}[htb!]
    \caption{\autoref{fig:dynamics} split by protest grievance.}
    \label{fig:g_dynamics}
    \begin{center}
    \begin{subfigure}{0.45\textwidth}\caption{Violent, Apex.}
        \includegraphics[width=\textwidth]{g_dynamics_violent_pa.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Peaceful, Apex.}
        \includegraphics[width=\textwidth]{g_dynamics_peaceful_pa.pdf}\end{subfigure}

    \begin{subfigure}{0.45\textwidth}\caption{Violent, Non-Apex.}
        \includegraphics[width=\textwidth]{g_dynamics_violent_na.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Peaceful, Non-Apex.}
        \includegraphics[width=\textwidth]{g_dynamics_peaceful_na.pdf}\end{subfigure}
    \end{center}
    \footnotesize{As \autoref{fig:dynamics}, but each panel overlays corruption-related protests (solid black line, circle markers) and unrelated protests (short-dashed grey line, square markers), estimated separately on the Apex and Non-Apex subsamples over the $\pm60$-day window in 15-day bins. The two series are dodged horizontally so their 90\% confidence spikes do not overlap; the bin immediately before disclosure is the omitted reference and the faint vertical band marks the cutoff. Standard errors clustered at country $\times$ year $\times$ day-bin. Sample period 2008--2019; Venezuela excluded.}
\end{figure}

\subsection{Specificity benchmarks}

\begin{table}[htb!]
    \caption{\autoref{tab:uniquetocorruption} split by protest grievance.}
    \label{tab:g_benchmarks}
    \begin{center}
        \scriptsize{\input{g_benchmarks_panels}}
    \end{center}
    \footnotesize{As \autoref{tab:uniquetocorruption}, but the Apex and Non-Apex samples are each split by grievance class, giving four corruption/rank panels; the football and currency-depreciation panels use the total protest counts, as in the main table. Each cell is the post-event OLS coefficient (standard error in parentheses) on the relevant outcome, at the $\pm30/60/90$-day windows, with country-year, month-of-the-year, and day-of-the-week fixed effects and clustering at the country $\times$ year $\times$ day-bin level. Sample period 2008--2019; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}

\subsection{Heterogeneity by the level of democracy}

\begin{table}[htb!]
    \caption{\autoref{tab:sup_democracy_lmh} split by protest grievance.}
    \label{tab:g_democracy}
    \begin{center}
        \scriptsize{\input{g_democracy_panels}}
    \end{center}
    \footnotesize{As \autoref{tab:sup_democracy_lmh} (the Low vs.\ Medium+High 2008-democracy split), re-estimated separately on corruption-related protests (Panel A) and unrelated protests (Panel B). Columns, fixed effects, clustering and sample as in \autoref{tab:g_interaction}. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}

\subsection{Sensitivity to the event-window width}

\begin{figure}[htb!]
    \caption{\autoref{fig:sup_window} split by protest grievance.}
    \label{fig:g_window}
    \begin{center}
    \begin{subfigure}{0.48\textwidth}\caption{Violent, Apex.}
        \includegraphics[width=\textwidth]{g_window_violent_pa.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.48\textwidth}\caption{Violent, Non-Apex.}
        \includegraphics[width=\textwidth]{g_window_violent_na.pdf}\end{subfigure} \\
    \begin{subfigure}{0.48\textwidth}\caption{Peaceful, Apex.}
        \includegraphics[width=\textwidth]{g_window_peaceful_pa.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.48\textwidth}\caption{Peaceful, Non-Apex.}
        \includegraphics[width=\textwidth]{g_window_peaceful_na.pdf}\end{subfigure}
    \end{center}
    \footnotesize{As \autoref{fig:sup_window}: the OLS post-event estimate against the event-window width $T\in\{15,30,\dots,150\}$, but each panel overlays corruption-related (solid black, circle markers) and unrelated (short-dashed grey, square markers) protests, with 90\% confidence bars. Panels share a common vertical scale. Sample period 2008--2019; Venezuela excluded.}
\end{figure}

\subsection{Regression discontinuity in event time}

\begin{table}[htb!]
    \caption{\autoref{tab:sup_rd_time} split by protest grievance.}
    \label{tab:g_rd_time}
    \begin{center}
        \scriptsize{\input{g_rd_time_panels}}
    \end{center}
    \footnotesize{As \autoref{tab:sup_rd_time} (the discontinuity $\tau$ at the disclosure cutoff), re-estimated separately on corruption-related protests (Panel A) and unrelated protests (Panel B), for the local-linear, local-quadratic, and nonparametric fits at $h=30/60/90$ days. Standard errors clustered at the country $\times$ year $\times$ day-bin level (nonparametric: robust bias-corrected). Sample period 2008--2019; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}

\begin{figure}[htb!]
    \caption{Regression discontinuity in event time, violent protests, split by grievance.}
    \label{fig:g_rd_violent}
    \begin{center}
    \begin{subfigure}{0.45\textwidth}\caption{Apex, $h=30$.}
        \includegraphics[width=\textwidth]{g_rd_violent_pa_h30.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $h=30$.}
        \includegraphics[width=\textwidth]{g_rd_violent_na_h30.pdf}\end{subfigure}

    \begin{subfigure}{0.45\textwidth}\caption{Apex, $h=60$.}
        \includegraphics[width=\textwidth]{g_rd_violent_pa_h60.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $h=60$.}
        \includegraphics[width=\textwidth]{g_rd_violent_na_h60.pdf}\end{subfigure}

    \begin{subfigure}{0.45\textwidth}\caption{Apex, $h=90$.}
        \includegraphics[width=\textwidth]{g_rd_violent_pa_h90.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $h=90$.}
        \includegraphics[width=\textwidth]{g_rd_violent_na_h90.pdf}\end{subfigure}
    \end{center}
    \footnotesize{Local-linear RD-in-event-time plots of the fixed-effect-residualised \emph{violent} protest count, overlaying corruption-related (black circles, solid black fits) and unrelated (grey squares, short-dashed grey fits) protests on each side of the disclosure cutoff, at $h=30/60/90$ days. Common vertical scale across all panels. Sample period 2008--2019; Venezuela excluded.}
\end{figure}

\begin{figure}[htb!]
    \caption{Regression discontinuity in event time, peaceful protests, split by grievance.}
    \label{fig:g_rd_peaceful}
    \begin{center}
    \begin{subfigure}{0.45\textwidth}\caption{Apex, $h=30$.}
        \includegraphics[width=\textwidth]{g_rd_peaceful_pa_h30.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $h=30$.}
        \includegraphics[width=\textwidth]{g_rd_peaceful_na_h30.pdf}\end{subfigure}

    \begin{subfigure}{0.45\textwidth}\caption{Apex, $h=60$.}
        \includegraphics[width=\textwidth]{g_rd_peaceful_pa_h60.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $h=60$.}
        \includegraphics[width=\textwidth]{g_rd_peaceful_na_h60.pdf}\end{subfigure}

    \begin{subfigure}{0.45\textwidth}\caption{Apex, $h=90$.}
        \includegraphics[width=\textwidth]{g_rd_peaceful_pa_h90.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}\caption{Non-Apex, $h=90$.}
        \includegraphics[width=\textwidth]{g_rd_peaceful_na_h90.pdf}\end{subfigure}
    \end{center}
    \footnotesize{As \autoref{fig:g_rd_violent}, but for \emph{peaceful} protests. Sample period 2008--2019; Venezuela excluded.}
\end{figure}

\subsection{Leave-one-out by scandal}

\begin{figure}[htb!]
    \caption{Leave-one-out by scandal, violent protests, split by grievance.}
    \label{fig:g_loo_violent}
    \begin{center}
    \begin{subfigure}{\textwidth}\caption{Apex, corruption-related.}
        \includegraphics[width=\textwidth]{g_loo_apex_gcorr_v.pdf}\end{subfigure} \\[4pt]
    \begin{subfigure}{\textwidth}\caption{Apex, unrelated.}
        \includegraphics[width=\textwidth]{g_loo_apex_gunrel_v.pdf}\end{subfigure} \\[4pt]
    \begin{subfigure}{\textwidth}\caption{Non-Apex, corruption-related.}
        \includegraphics[width=\textwidth]{g_loo_nonapex_gcorr_v.pdf}\end{subfigure} \\[4pt]
    \begin{subfigure}{\textwidth}\caption{Non-Apex, unrelated.}
        \includegraphics[width=\textwidth]{g_loo_nonapex_gunrel_v.pdf}\end{subfigure}
    \end{center}
    \footnotesize{As \autoref{fig:sup_loo}, but for the grievance-split \emph{violent} outcomes: leave-one-out estimates of the Post $\times$ Apex (top two panels) and Post $\times$ Non-Apex (bottom two) coefficients on corruption-related and unrelated violent protests, $\pm30$-day window, dropping each scandal in turn. Vertical bars are 90\% confidence intervals; the red line is the coefficient on all scandals. Standard errors clustered at country $\times$ year $\times$ day-bin. Sample period 2008--2019; Venezuela excluded.}
\end{figure}

\begin{figure}[htb!]
    \caption{Leave-one-out by scandal, peaceful protests, split by grievance.}
    \label{fig:g_loo_peaceful}
    \begin{center}
    \begin{subfigure}{\textwidth}\caption{Apex, corruption-related.}
        \includegraphics[width=\textwidth]{g_loo_apex_gcorr_p.pdf}\end{subfigure} \\[4pt]
    \begin{subfigure}{\textwidth}\caption{Apex, unrelated.}
        \includegraphics[width=\textwidth]{g_loo_apex_gunrel_p.pdf}\end{subfigure} \\[4pt]
    \begin{subfigure}{\textwidth}\caption{Non-Apex, corruption-related.}
        \includegraphics[width=\textwidth]{g_loo_nonapex_gcorr_p.pdf}\end{subfigure} \\[4pt]
    \begin{subfigure}{\textwidth}\caption{Non-Apex, unrelated.}
        \includegraphics[width=\textwidth]{g_loo_nonapex_gunrel_p.pdf}\end{subfigure}
    \end{center}
    \footnotesize{As \autoref{fig:g_loo_violent}, but for \emph{peaceful} protests. Sample period 2008--2019; Venezuela excluded.}
\end{figure}

\subsection{Randomization inference}

The grievance-split randomization-inference histograms (one per grievance outcome, Apex/Non-Apex $\times$ $\pm30/60/90$) are produced by \texttt{g\_randomization\_inference.do}. Across the four grievance outcomes the placebo distributions are centred near zero; the observed Apex violent-protest estimates (corruption-related and unrelated) sit in the right tail, while the peaceful and non-apex estimates fall inside the placebo mass---mirroring \autoref{sup:ri}.

\begin{figure}[htb!]
    \caption{Randomization inference: corruption-related violent protests.}
    \label{fig:g_ri_corr_violent}
    \begin{center}
    \begin{subfigure}{0.32\textwidth}\caption{Apex, $\pm30$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gcorr_v_w30_pa.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Apex, $\pm60$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gcorr_v_w60_pa.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Apex, $\pm90$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gcorr_v_w90_pa.pdf}\end{subfigure} \\
    \begin{subfigure}{0.32\textwidth}\caption{Non-Apex, $\pm30$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gcorr_v_w30_na.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Non-Apex, $\pm60$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gcorr_v_w60_na.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Non-Apex, $\pm90$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gcorr_v_w90_na.pdf}\end{subfigure}
    \end{center}
    \footnotesize{As \autoref{fig:sup_ri_violent}, but for corruption-related violent protests: each histogram is the distribution of 1{,}000 placebo estimates (disclosure dates redrawn within country-year); the maroon line marks the observed estimate and the legend reports the two-sided randomization-inference $p$-value. Sample period 2008--2019; Venezuela excluded.}
\end{figure}

\begin{figure}[htb!]
    \caption{Randomization inference: unrelated violent protests.}
    \label{fig:g_ri_unrel_violent}
    \begin{center}
    \begin{subfigure}{0.32\textwidth}\caption{Apex, $\pm30$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gunrel_v_w30_pa.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Apex, $\pm60$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gunrel_v_w60_pa.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Apex, $\pm90$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gunrel_v_w90_pa.pdf}\end{subfigure} \\
    \begin{subfigure}{0.32\textwidth}\caption{Non-Apex, $\pm30$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gunrel_v_w30_na.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Non-Apex, $\pm60$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gunrel_v_w60_na.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Non-Apex, $\pm90$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gunrel_v_w90_na.pdf}\end{subfigure}
    \end{center}
    \footnotesize{As \autoref{fig:g_ri_corr_violent}, but for unrelated violent protests.}
\end{figure}

\begin{figure}[htb!]
    \caption{Randomization inference: corruption-related peaceful protests.}
    \label{fig:g_ri_corr_peaceful}
    \begin{center}
    \begin{subfigure}{0.32\textwidth}\caption{Apex, $\pm30$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gcorr_p_w30_pa.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Apex, $\pm60$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gcorr_p_w60_pa.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Apex, $\pm90$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gcorr_p_w90_pa.pdf}\end{subfigure} \\
    \begin{subfigure}{0.32\textwidth}\caption{Non-Apex, $\pm30$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gcorr_p_w30_na.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Non-Apex, $\pm60$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gcorr_p_w60_na.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Non-Apex, $\pm90$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gcorr_p_w90_na.pdf}\end{subfigure}
    \end{center}
    \footnotesize{As \autoref{fig:g_ri_corr_violent}, but for corruption-related peaceful protests.}
\end{figure}

\begin{figure}[htb!]
    \caption{Randomization inference: unrelated peaceful protests.}
    \label{fig:g_ri_unrel_peaceful}
    \begin{center}
    \begin{subfigure}{0.32\textwidth}\caption{Apex, $\pm30$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gunrel_p_w30_pa.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Apex, $\pm60$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gunrel_p_w60_pa.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Apex, $\pm90$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gunrel_p_w90_pa.pdf}\end{subfigure} \\
    \begin{subfigure}{0.32\textwidth}\caption{Non-Apex, $\pm30$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gunrel_p_w30_na.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Non-Apex, $\pm60$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gunrel_p_w60_na.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}\caption{Non-Apex, $\pm90$.}
        \includegraphics[width=\textwidth]{g_ri_hist_gunrel_p_w90_na.pdf}\end{subfigure}
    \end{center}
    \footnotesize{As \autoref{fig:g_ri_corr_violent}, but for unrelated peaceful protests.}
\end{figure}
