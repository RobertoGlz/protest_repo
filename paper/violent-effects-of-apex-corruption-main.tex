...
\begin{figure}[htb!]
    \caption{Protest dynamics by apex-partition subsample ($\pm 60$-day window, 15-day bins).}
    \label{fig:dynamics}
    \begin{center}
    \begin{subfigure}{0.45\textwidth}
        \caption{Violent, Apex.}
        \includegraphics[width=\textwidth]{es_num_violent_MM_w60_b15_pa_ols_90ci.pdf}
        \label{fig:dynamics:pav}
    \end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}
        \caption{Peaceful, Apex.}
        \includegraphics[width=\textwidth]{es_num_peaceful_MM_w60_b15_pa_ols_90ci.pdf}
        \label{fig:dynamics:pap}
    \end{subfigure} \\
    \begin{subfigure}{0.45\textwidth}
        \caption{Violent, Non-Apex.}
        \includegraphics[width=\textwidth]{es_num_violent_MM_w60_b15_na_ols_90ci.pdf}
        \label{fig:dynamics:nav}
    \end{subfigure}\hfill
    \begin{subfigure}{0.45\textwidth}
        \caption{Peaceful, Non-Apex.}
        \includegraphics[width=\textwidth]{es_num_peaceful_MM_w60_b15_na_ols_90ci.pdf}
        \label{fig:dynamics:nap}
    \end{subfigure}
    \end{center}
    \footnotesize{OLS estimates on event-time 15-day-bin indicators from the modified \autoref{eq:main}, estimated separately on the Apex subsample (top row) and the Non-Apex subsample (bottom row). The reference bin---the 15 days immediately preceding disclosure ($w_{s,t}\in[-15,-1]$)---is normalized to zero and plotted at its position; the faint thick vertical line marks the disclosure cutoff. All four panels share a common y-axis. Vertical bars are 90\% confidence intervals; standard errors clustered at country $\times$ year $\times$ day-bin. An observation is a country-scandal-date. Sample period 2008--2018; Venezuela excluded.}
\end{figure}
...
\paragraph{Design 1 (stacked by scandal).} The design closest to our main event-window specification is a Cengiz-style stack~\citep{cengiz2019effect}: for each scandal we take the treated country's $\pm 120$-day (15-day-bin) window together with every country with no scandal of any type in that window as a clean control cohort, and pool the stacks. Table~\ref{tab:sup_did_stk} reports the static post-scandal coefficient from each estimator, split into Apex (Panel A) and Non-Apex (Panel B); Figure~\ref{fig:sup_did_stk_es} overlays the four estimators' event-study paths for violent protests.
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{Event-study coefficients on violent protests from the four estimators overlaid, on the stacked-by-scandal panel; time axis in 15-day bins (labeled in days), reference bin $-1$. Vertical bars are 90\% confidence intervals; SE clustered at the country level.}
\end{figure}
...
\begin{table}[htb!]
    ...
    \begin{center}
        \textbf{Panel A: Apex ($\pm 60$)}\\[3pt]
        \scriptsize{\input{did_modern_main_pa_w60}}\\[6pt]
        \textbf{Panel B: Non-Apex ($\pm 60$)}\\[3pt]
        \scriptsize{\input{did_modern_main_na_w60}}\\[10pt]
        \textbf{Panel C: Apex ($\pm 90$)}\\[3pt]
        \scriptsize{\input{did_modern_main_pa_w90}}\\[6pt]
        \textbf{Panel D: Non-Apex ($\pm 90$)}\\[3pt]
        \scriptsize{\input{did_modern_main_na_w90}}\\[10pt]
        \textbf{Panel E: Apex ($\pm 120$)}\\[3pt]
        \scriptsize{\input{did_modern_main_pa_w120}}\\[6pt]
        \textbf{Panel F: Non-Apex ($\pm 120$)}\\[3pt]
        \scriptsize{\input{did_modern_main_na_w120}}
    \end{center}
    \footnotesize{Static post-treatment coefficient (standard error) from OLS TWFE, dCDH, BJS, and SA on the country $\times$ 15-day-bin staggered panel, treatment defined as the country's first-scandal bin (never-treated cohort $=$ countries with no subsample scandal), at the $\pm 60$/$\pm 90$/$\pm 120$-day windows (4/6/8 bins per side). Columns are the four Mass-Mobilization outcomes. Standard errors clustered at the country level. Sample period 2008--2018. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{Event-study coefficients on violent protests from the four estimators overlaid, on the country $\times$ 15-day-bin staggered panel; time axis in days (15-day bins), reference bin $-1$ (the 15 days before disclosure). Vertical bars are 90\% confidence intervals; SE clustered at the country level.}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{Same construction as \autoref{fig:sup_did_cw_es}, at the $\pm 90$-day window (6 bins per side).}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{Same construction as \autoref{fig:sup_did_cw_es}, at the $\pm 120$-day window (8 bins per side).}
\end{figure}
...
The main text reports the event study on the $\pm 120$-day window with 15-day bins (\autoref{fig:dynamics}). Figures~\ref{fig:sup_es_w30}--\ref{fig:sup_es_w90} repeat the exercise at the three narrower widths, $T \in \{30, 60, 90\}$ days, keeping 15-day bins throughout. Within each figure all four panels---violent and peaceful, Apex and Non-Apex---share a common y-axis range, so the estimates can be compared by eye and the zero line is aligned across panels.
