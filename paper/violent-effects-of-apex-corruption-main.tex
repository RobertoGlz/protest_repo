...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{OLS event study estimated \emph{separately} on the Apex and Non-Apex subsamples, replacing the post-scandal indicator with a saturated set of 15-day event-time bin indicators over the $\pm 60$-day window (\autoref{eq:main}). The four panels are violent and peaceful protests $\times$ Apex and Non-Apex; the bin immediately preceding disclosure ($w_{s,t}\in[-15,-1]$) is the omitted reference and the faint vertical band marks the disclosure cutoff. All four panels share a common y-axis. Vertical bars are 90\% confidence intervals; standard errors clustered at country $\times$ year $\times$ day-bin. Sample period 2008--2018; Venezuela excluded.}
\end{figure}
...
\autoref{tab:sup_democracy_lmh} groups the panel of countries into terciles of their 2008 level of democracy---measured by V-Dem's Electoral Democracy Index~\citep{vdem2024}---and interacts the post-scandal indicator with a pooled top-two-tercile (``Medium+High'') versus bottom-tercile (``Low'') classification. The violent-protest effect is larger in the Medium+High group at every window and the one-sided ordering $\beta_{\text{Medium+High}}>\beta_{\text{Low}}$ strengthens at the wider windows---so, to the extent the heterogeneity is real, it is the \emph{higher}-democracy countries that drive the effect. We cannot always statistically distinguish the two groups, however, so this heterogeneity is suggestive rather than conclusive.
...
\begin{table}[htb!]
    ...
    \end{center}
    \footnotesize{OLS estimates of $\beta_{\text{Medium+High}}$ and $\beta_{\text{Low}}$, the post-scandal indicator interacted with a top-two-tercile (``Medium+High'') and a bottom-tercile (``Low'') flag from terciles of V-Dem's 2008 Electoral Democracy Index (\texttt{v2x\_polyarchy}). Columns are violent and peaceful protest counts at the $\pm$30/60/90-day windows. ``$p$-value: Medium+High $=$ Low'' reports the two-sided $p$-value for $H_0:\beta_{\text{Medium+High}}=\beta_{\text{Low}}$; ``$p$-value: Medium+High $>$ Low (one-sided)'' reports the one-sided $p$-value. Same fixed effects, clustering, and sample as Table~\ref{tab:main}. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}
...
Each of the 176 scandals in our event-window panel is manually classified by the position of the implicated politician, in two steps. \emph{Step 1 (raw position).} Starting from the free-text description of each scandal, a research assistant assigns one of five raw codes: \texttt{president} (sitting or immediately-former head of state); \texttt{governor} (head of a first-order subnational unit); \texttt{sc\_judge\_congressman} (a Supreme Court Justice \emph{or} a national legislator); \texttt{other\_judiciary} (lower-court judges, prosecutors); and \texttt{others} (ministers, agency heads, mayors, and all remaining officials). \emph{Step 2 (splitting \texttt{sc\_judge\_congressman}).} Because that raw code lumps constitutional-court justices together with members of Congress, we verify each such row and tag the four scandals implicating a Supreme Court justice (they appear as \emph{Supreme Court justice} in the \textit{Role} column of \autoref{tab:sup_scandal_list}). We then aggregate into three bins: \textbf{President} $=$ \texttt{president} (45 scandals); \textbf{Other Apex} $=$ \texttt{governor} plus the four Supreme Court justices (18 scandals); and \textbf{Non-Apex} $=$ \texttt{other\_judiciary} $+$ \texttt{others} $+$ the remaining \texttt{sc\_judge\_congressman} rows (Congressmen; 113 scandals). Throughout the paper ``Apex'' pools the President and Other Apex bins (63 scandals).

\autoref{tab:sup_scandal_list} lists every scandal in the panel, numbered by its order of occurrence (chronological, by disclosure date), together with its country, year, the role of the implicated official, a description, and its Apex/Non-Apex classification. This order-of-occurrence number---rather than any internal case identifier---is what labels each scandal in the leave-one-out figures of \autoref{sup:loo}, so a marker there can be traced back to its row in the table. One borderline case, a 2017 Mexican scandal, was originally re-tagged as a Supreme Court justice, but its source describes a San Luis Potos\'i case implicating \emph{diputados} (members of Congress); we therefore classify it as Non-Apex. Four scandals had blank descriptions in the classification file; we recovered them from the underlying news source and report them in the table.
...
\autoref{fig:dynamics} in the main text reports the subsample event study---estimated separately on the Apex and Non-Apex samples---at the $\pm 60$-day window. This section reports the same specification at the $\pm 30$ and $\pm 90$-day windows, keeping 15-day bins throughout. Within each figure all four panels---violent and peaceful, Apex and Non-Apex---share a common y-axis range, so the estimates can be compared by eye and the zero line is aligned across panels.
