...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{One panel per scandal country (the 16 countries in the paper's democracy tables). Solid black line: the country's V-Dem Electoral Democracy Index (\texttt{v2x\_polyarchy}) by year. Gray vertical lines: the year-month of each apex-corruption scandal in that country (Bolivia, Chile, Costa Rica and Nicaragua have no apex scandals, hence no vertical lines). Short-dashed lines: the mean index of the bottom (cranberry) and top (navy) tercile of the 2008 Electoral Democracy Index (\texttt{xtile}, \texttt{nq(3)})---the same tercile classification used in the paper's democracy tables---drawn in every panel for reference. Common vertical scale. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{As \autoref{fig:sup_dem_scandals}, but the solid line and the two tercile-mean reference lines use V-Dem's Liberal Democracy Index (\texttt{v2x\_libdem}), with terciles formed from its 2008 value. Gray vertical lines mark apex-scandal year-months. Common vertical scale. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
