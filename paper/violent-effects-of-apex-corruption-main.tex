...
As a descriptive complement to the cross-sectional democracy split, \autoref{fig:sup_dem_scandals} tracks apex-scandal accumulation and the level of democracy over time. For the countries in which our scandals occur, split into the bottom and top tercile of the 2008 V-Dem Electoral Democracy Index (the same 2008 tercile classification used in the paper's democracy tables), it plots the cumulative number of apex scandals the average country has experienced (left axis) alongside the tercile-mean Electoral Democracy Index (right axis). In both groups apex scandals accumulate steeply after 2014 while the democracy index stays flat until about 2016 and then declines---so scandal accumulation \emph{precedes} the democracy dip. This is a country-aggregated description, not a causal estimate.
...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{The scandal countries are split into the bottom (6 countries) and top (5 countries) tercile of the 2008 V-Dem Electoral Democracy Index (\texttt{v2x\_polyarchy}), by \texttt{xtile}, \texttt{nq(3)}---the same classification used in the paper's democracy heterogeneity tables; the middle tercile is omitted. Solid lines (left axis) are the cumulative count of apex-corruption scandals experienced by the average country in the tercile since 2008; dashed lines (right axis) are the tercile-mean Electoral Democracy Index. Cranberry is the bottom tercile, navy the top. Apex scandals accumulate steeply after 2014 in both groups, while the democracy index is flat until about 2016 and then falls (top tercile from about 0.85 to 0.81, bottom tercile from about 0.55 to 0.50), so scandal accumulation precedes the democracy decline. Descriptive only. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
