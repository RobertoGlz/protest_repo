...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{This table shows estimates of our OLS specification for each window-width $T \in \{15, 30, \dots, 150\}$. Vertical bars are 90\% confidence intervals; the estimates at $T=30, 60, 90$ (the widths in Table~\ref{tab:uniquetocorruption}) are shown with red diamonds. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
...
We show that our results are robust to a discontinuity-in-time design, using the number of days since/to a given scandal as the running variable and setting the disclosure date as the cutoff value. We allow the polynomial on each side of the cutoff to differ in shape but not in degree. We partial-out month-of-the-year, day-of-the-week and country-year fixed effects.

\autoref{tab:sup_rd_time} reports the estimated immediate effect for the apex and non-apex subsamples, on violent and peaceful protests, for bandwidths $h\in\{30,60,90\}$ days and local-linear, local-quadratic polynomials, and a non-parametric fit. \autoref{fig:sup_rd_time} plots the two violent-protest discontinuities: for apex scandals the residualised protest count jumps sharply upward at disclosure, whereas for non-apex scandals the fit is continuous through the cutoff. 
...
\begin{table}[htb!]
    ...
    \end{center}
    \footnotesize{This table shows estimates of $\tau$ in $Y_{c(s)t} = \tau\,\text{Post}_{st} + f(w_{s,t}) + \text{Post}_{st}\cdot g(w_{s,t}) + \alpha_d + \lambda_m + \theta_{cy} + \varepsilon_{c(s)t}$, where the polynomials $f,g$ are local-linear or local-quadratic in the running variable and, the running variable being centred at $0$. The ``Nonparametric'' rows report estimates using a local-polynomial with MSE-optimal data-driven bandwidth and a triangular kernel, computed within the $\pm h$ window on the partialed-out outcome. We estimate each effect separately on each of the Apex and Non-Apex subsamples, on violent and peaceful protest counts, and at windows $h\in\{30,60,90\}$ days. All specifications include country-by-year, month-of-year, and day-of-week fixed effects; standard errors (in parentheses) clustered at the country $\times$ year $\times$ day-bin level. Sample period 2008--2019; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}
