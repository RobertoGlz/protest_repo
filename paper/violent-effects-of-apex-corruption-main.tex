...
\begin{table}[htb!]
    ...
    \begin{center}
        \scriptsize{\input{sup_benchmarks_ols_panels_shares}}
    \end{center}
...
Each of the 176 scandals in our event-window panel is manually classified by the position of the implicated politician, in two steps. \emph{Step 1 (raw position).} Starting from the free-text description of each scandal, a research assistant assigns one of five raw codes: \texttt{president} (sitting or immediately-former head of state); \texttt{governor} (head of a first-order subnational unit); \texttt{sc\_judge\_congressman} (a Supreme Court Justice \emph{or} a national legislator); \texttt{other\_judiciary} (lower-court judges, prosecutors); and \texttt{others} (ministers, agency heads, mayors, and all remaining officials). \emph{Step 2 (splitting \texttt{sc\_judge\_congressman}).} Because that raw code lumps constitutional-court justices together with members of Congress, we verify each such row and tag the ones implicating a Supreme Court justice; the re-tagged IDs are \texttt{202}, \texttt{NEW26}, \texttt{NEW30}, \texttt{332}, and \texttt{NEW23}. We then aggregate into three bins: \textbf{President} $=$ \texttt{president} (45 scandals); \textbf{Other Apex} $=$ \texttt{governor} plus the five SC-Justice IDs (19 scandals); and \textbf{Other Non-Apex} $=$ \texttt{other\_judiciary} $+$ \texttt{others} $+$ the remaining \texttt{sc\_judge\_congressman} IDs (Congressmen; 112 scandals). Throughout the paper ``Apex'' pools the President and Other Apex bins (64 scandals).

% ---------------------------------------------------------------------
\clearpage
\section{Poisson QML estimates}\label{sup:poisson}

Because the outcome is a non-negative count, we complement the OLS estimates with Poisson Quasi-maximum Likelihood (QML) estimation of \autoref{eq:main} (\texttt{ppmlhdfe}), which reports multiplicative effects as incidence-rate ratios (IRR $=\exp\beta$). Table~\ref{tab:sup_poisson} is the Poisson counterpart to Table~\ref{tab:main}: same two-panel split, same $\pm 30$/$\pm 120$-day windows, fixed effects, and clustering. Poisson is estimated on the four count outcomes only; the share columns are blank because an IRR has no meaning for a share in $[0,1]$. The incidence-rate ratios confirm the OLS pattern---in the Apex panel violent protests more than double in the $\pm 30$-day window and stay elevated at $\pm 120$ days, while the Other Non-Apex panel and the peaceful outcomes stay near one.

\begin{table}[htb!]
    \caption{Table~\ref{tab:main} with Poisson QML incidence-rate ratios.}
    \label{tab:sup_poisson}
    \begin{center}
        \scriptsize{\input{sup_violent_peaceful_poi_panels}}
    \end{center}
    \footnotesize{Poisson QML incidence-rate ratios $\exp(\beta)$ from \autoref{eq:main} with delta-method standard errors in parentheses; stars test $H_0:\beta=0$ (IRR $=1$). Panel A: President + Other Apex (64 scandals); Panel B: Other Non-Apex (112 scandals). Columns 1--4 report the four count outcomes; share columns are intentionally blank. Standard errors clustered at the country $\times$ year $\times$ day-bin level; same fixed effects, sample period, and clustering as Table~\ref{tab:main}. The Poisson sample is smaller than the OLS sample because \texttt{ppmlhdfe} drops singleton and separated observations. Sample period 2008--2018; Venezuela excluded.}
\end{table}

% ---------------------------------------------------------------------
\clearpage
\section{Restricted subsamples: Presidential and President + Other Apex}\label{sup:restricted}

Tables~\ref{tab:sup_pres_only} and \ref{tab:sup_pres_apex} re-estimate \autoref{eq:main} on progressively narrower subsamples: scandals implicating a sitting President only (45 scandals), and Presidents plus Other Apex officials (64 scandals). Each table reports OLS on the eight outcomes (Panel A) and Poisson QML incidence-rate ratios on the four count outcomes (Panel B). The violent-protest effect is present in the President-only sample and remains when Other Apex scandals are added.
...
\begin{equation*}
    ...
    \;+\; \alpha_d + \lambda_m + \theta_{cy} + \varepsilon_{c(s)t},
\end{equation*}
where $\mathrm{Pres}_s$, $\mathrm{OA}_s$, $\mathrm{ONA}_s$ are scandal-level indicators for President, Other Apex, and Other Non-Apex. The specification omits a stand-alone Post term, so each coefficient is the average post-scandal effect for that category, and we report the $p$-values from pairwise equality tests.
...
\begin{table}[htb!]
    ...
    \end{center}
    \footnotesize{OLS estimates of $\beta_{\text{Pres}}$, $\beta_{\text{OA}}$, $\beta_{\text{ONA}}$ (Post $\times$ President, Other Apex, Other Non-Apex) on the four count outcomes (cols.\ 1--4) and four share outcomes (cols.\ 5--8). Standard errors (in parentheses) clustered at the country $\times$ year $\times$ day-bin level. Fixed effects, sample period, and clustering identical to Table~\ref{tab:main}. Rows labeled ``$p$-value'' report pairwise equality tests across the three category coefficients.}
\end{table}
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \footnotesize{For each offset $k$ we replace the treatment indicator by $\widetilde{\mathrm{Post}}_{st}=\mathbf{1}\{w_{st}\geq k\}$ and re-estimate \autoref{eq:main}. Gray circles are the resulting $\hat\beta(k)$; the solid red line is the observed $\hat\beta$ from Table~\ref{tab:main}; the legend gives the rank of the observed estimate and the two-sided $p$-value. SE clustered at country $\times$ year $\times$ day-bin. Sample period 2008--2018; Venezuela excluded.}
\end{figure}

\begin{figure}[htb!]
    \caption{Placebo test, President + Other Apex subsample.}
    ...
    \end{subfigure}
    \footnotesize{Same construction as \autoref{fig:sup_placebo}, restricted to the 64 President + Other Apex scandals; observed $\hat\beta$ from Panel A of Table~\ref{tab:main}.}
\end{figure}

\begin{figure}[htb!]
    \caption{Placebo test, Other Non-Apex subsample.}
    ...
    \end{subfigure}
    \footnotesize{Same construction as \autoref{fig:sup_placebo}, restricted to the 112 Other Non-Apex scandals; observed $\hat\beta$ from Panel B of Table~\ref{tab:main}.}
\end{figure}
...
% ---------------------------------------------------------------------
\clearpage
\section{Poisson QML counterparts: benchmarks and democracy split}\label{sup:poisson_more}

Tables~\ref{tab:sup_benchmarks_poi} and \ref{tab:sup_democracy_poi} report the Poisson QML incidence-rate-ratio counterparts to the benchmark table (Table~\ref{tab:benchmarks}) and the democracy split (Table~\ref{tab:democracy}) of the main text, on the four count outcomes.
...
\begin{table}[htb!]
    ...
    \end{center}
    \footnotesize{Poisson QML incidence-rate ratios $\exp(\beta)$; stars test $H_0:\beta=0$ (IRR $=1$). Panels and specification as in Table~\ref{tab:benchmarks} (Panel A apex corruption; Panel B non-apex corruption; Panel C football losses; Panel D depreciations). Sample period 2008--2018; Venezuela excluded.}
\end{table}
...
\begin{table}[htb!]
    ...
    \end{center}
    \footnotesize{Poisson QML incidence-rate ratios for the two interaction terms (Post $\times$ above-/at-or-below-median 2008 democracy), count outcomes only. Split, fixed effects, clustering, and sample as in Table~\ref{tab:democracy}.}
\end{table}

% ---------------------------------------------------------------------
\clearpage
\section{Event studies, Poisson QML}\label{sup:es_poi}

Figure~\ref{fig:sup_es_poi} reports the Poisson QML incidence-rate-ratio counterpart to the main-text event studies (\autoref{fig:dynamics}), estimated separately on the two subsamples.
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{Poisson QML incidence-rate ratios $\exp(\hat\beta)$ on event-time 30-day-bin indicators, estimated separately on each subsample (reference bin normalized to an IRR of 1); the dashed line marks IRR $=1$. Vertical bars are 90\% confidence intervals. Same fixed effects, clustering, and sample as \autoref{fig:dynamics}.}
\end{figure}

% ---------------------------------------------------------------------
\clearpage
\section{Window-width sensitivity, Poisson QML}\label{sup:window_poi}

Figures~\ref{fig:sup_window_poi_pa} and \ref{fig:sup_window_poi_na} report the Poisson QML incidence-rate-ratio counterpart to the window-width sweep of \autoref{sup:window}.

\begin{figure}[htb!]
    \caption{Window-width sensitivity (Poisson QML IRR), President + Other Apex subsample.}
    \label{fig:sup_window_poi_pa}
    \begin{center}
    \begin{subfigure}{0.48\textwidth}
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{Poisson QML incidence-rate ratio $\exp(\hat\beta)$ at each event-window width $T$, on the 64 President + Other Apex scandals. Vertical bars are 90\% confidence intervals; $T=30$ and $T=120$ highlighted; the thick line marks IRR $=1$. Y-axis shared between panels. Sample period 2008--2018; Venezuela excluded.}
\end{figure}

\begin{figure}[htb!]
    \caption{Window-width sensitivity (Poisson QML IRR), Other Non-Apex subsample.}
    \label{fig:sup_window_poi_na}
    \begin{center}
    \begin{subfigure}{0.48\textwidth}
        \caption{Violent protests.}
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{Same construction as \autoref{fig:sup_window_poi_pa}, restricted to the 112 Other Non-Apex scandals.}
\end{figure}
...
\paragraph{Design 1 (stacked by scandal).} The design closest to our main event-window specification is a Cengiz-style stack~\citep{cengiz2019effect}: for each scandal we take the treated country's $\pm 30$-day (3-day-bin) window together with every country with no scandal of any type in that window as a clean control cohort, and pool the stacks. Table~\ref{tab:sup_did_stk} reports the static post-scandal coefficient from each estimator, split into President + Other Apex (Panel A) and Other Non-Apex (Panel B); Figure~\ref{fig:sup_did_stk_es} overlays the four estimators' event-study paths for violent protests.

\paragraph{Design 2 (country-week first-scandal).} As a complementary check we collapse the data to a balanced country-week panel and define treatment as a country's \emph{first} scandal week, letting each estimator use its native never-treated control group. Table~\ref{tab:sup_did_cw} and Figure~\ref{fig:sup_did_cw_es} report the analogues. Across both designs and all three estimators, the violent-protest response in the President + Other Apex subsample is of the same sign and broad magnitude as the headline estimates, while the Other Non-Apex subsample stays flat---so the main result is not an artifact of TWFE aggregation. (Outcomes are Mass-Mobilization counts: ``Violent'' $=$ \texttt{mm\_violent}, ``Non-violent'' $=$ \texttt{mm\_nonviolent}.)
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{Event-study coefficients on violent protests from the four estimators overlaid, on the stacked-by-scandal panel; time axis in 3-day bins (labeled in days), reference bin $-1$. Vertical bars are 90\% confidence intervals; SE clustered at the country level.}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{Event-study coefficients on violent protests from the four estimators overlaid, on the country-week first-scandal panel; time axis in weeks since the first scandal, reference week $-1$. Vertical bars are 90\% confidence intervals; SE clustered at the country level.}
\end{figure}
