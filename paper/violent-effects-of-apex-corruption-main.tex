...
Our identification rests on the exact calendar date of a scandal's disclosure being unpredictable \emph{within} a country-year: a citizen may know that a given country-year is scandal-prone, but not the specific day a scoop will break. \autoref{tab:scandal_random} tests this directly. On a country-by-day panel of the sample countries, we ask whether the day a scandal is disclosed can be told apart from the other days of the \emph{same} country-year using calendar structure and the electoral cycle---the day of the week, the month, the day of the month, and proximity to a national election (0--30/31--60/61--90/91--120 days before). We estimate a conditional (country-by-year fixed-effects) logit and measure discrimination with the within-country-year AUC: the probability the model ranks the true scandal-day above a random non-scandal-day of the same country-year, where $0.5$ is a coin flip.

Out of sample---training on a random half of the days and ranking the held-out half---the within-country-year AUC never exceeds $0.59$, and falls to $0.54$ once all four predictor sets are included (\autoref{tab:scandal_random}, column~4): essentially a coin flip. Tellingly, adding predictors \emph{raises} the in-sample AUC (from $0.59$ to $0.68$) while \emph{lowering} the out-of-sample AUC---the signature of fitting noise rather than signal. Calendar structure and the electoral cycle carry no usable information about which day a scandal breaks.
...
\begin{table}[htb!]
    ...
    \footnotesize{Each column is a conditional (country-by-year fixed-effects) logit of a scandal-disclosure indicator on the listed predictors, estimated on a country-by-day panel of the sample countries (2008--2019, the span of our scandal dates; Venezuela excluded). Conditioning on country-by-year means only the 79 country-years that contain a scandal (28{,}852 country-days) enter the comparison; our 176 scandals fall on 171 distinct country-days. ``Election proximity'' is a set of indicators for a day falling 0--30, 31--60, 61--90, or 91--120 days before a national election. ``Joint test $p$-value'' is the Wald test that the added predictors jointly matter. ``Within-year AUC'' is Harrell's $c$ restricted to within-country-year comparisons (the probability the true scandal-day outranks a random non-scandal-day of the same country-year); $0.5$ is a coin flip. The out-of-sample row trains on a random half of the country-days and evaluates on the held-out half.}
\end{table}
