...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{As \autoref{fig:sup_rd_time}, but for peaceful protest counts. Binned means residualised on the country-by-year, month-of-year, and day-of-week fixed effects, in 5-day bins of the running variable, with local-linear fits on each side of the disclosure cutoff, at the $h=30$ (top row), $h=60$ (middle), and $h=90$ (bottom) bandwidths. Neither the apex (left column) nor the non-apex (right column) fit shows a discontinuity at $w_{s,t}=0$. Common vertical scale across all regression-discontinuity figures. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{As \autoref{fig:sup_rd_time_quad}, but for peaceful protest counts, with local-\emph{quadratic} fits on each side of the disclosure cutoff. Neither the apex (left column) nor the non-apex (right column) fit jumps at $w_{s,t}=0$. Common vertical scale across all regression-discontinuity figures. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{As \autoref{fig:sup_rd_time_np}, but for peaceful protest counts: local-linear fits (triangular kernel, MSE-optimal data-driven bandwidth) with 90\% confidence bands and binned means (12 per side), residualised on the country-by-year, month-of-year, and day-of-week fixed effects. The legend reports the discontinuity estimate $\tau$ and its $p$-value; both the apex and non-apex estimates are null. Common vertical scale across all regression-discontinuity figures. Sample period 2008--2019; Venezuela excluded.}
\end{figure}
