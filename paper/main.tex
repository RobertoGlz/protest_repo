...
%     do-files write); Protest_Work/results/ is only a fallback for outputs
%     not regenerated locally.  (Previously Protest_Work was searched first,
%     which shadowed freshly-regenerated tables/figures with stale copies.)
...
Importantly, our recolection of scandals allows us to identify the position of the implicated politician. We then look at the scandals and manually classify them in three categories: (i) ``President'' includes scandals in which the implicated official is a President (current or former), (ii) ``Other Apex'' includes scandals in which the implicated official is either a Governor or Supreme Court Justice, and (iii) ``Non-Apex'' includes scandals in which the implicated official is a Congressman, lower judiciary, or other lower-rank officials. Throughout, we group the President and Other Apex scandals together (``apex''; 63 scandals) and contrast them with the Non-Apex scandals (113 scandals).
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{For each $T \in \{15, 30, \dots, 150\}$ we re-estimate the headline OLS specification on the event-window panel restricted to $|w_{st}|\leq T$, separately on the 63 Apex and 113 Non-Apex scandals. Vertical bars are 90\% confidence intervals; the markers at $T=30, 60, 90, 120$ (the widths in Table~\ref{tab:main}) are highlighted in red. All four panels share a common y-axis so Apex and Non-Apex are directly comparable. Sample period 2008--2018; Venezuela excluded.}
\end{figure}
...
Each of the 176 scandals in our event-window panel is manually classified by the position of the implicated politician, in two steps. \emph{Step 1 (raw position).} Starting from the free-text description of each scandal, a research assistant assigns one of five raw codes: \texttt{president} (sitting or immediately-former head of state); \texttt{governor} (head of a first-order subnational unit); \texttt{sc\_judge\_congressman} (a Supreme Court Justice \emph{or} a national legislator); \texttt{other\_judiciary} (lower-court judges, prosecutors); and \texttt{others} (ministers, agency heads, mayors, and all remaining officials). \emph{Step 2 (splitting \texttt{sc\_judge\_congressman}).} Because that raw code lumps constitutional-court justices together with members of Congress, we verify each such row and tag the ones implicating a Supreme Court justice; the re-tagged IDs are \texttt{202}, \texttt{NEW26}, \texttt{NEW30}, and \texttt{332}. We then aggregate into three bins: \textbf{President} $=$ \texttt{president} (45 scandals); \textbf{Other Apex} $=$ \texttt{governor} plus the four SC-Justice IDs (18 scandals); and \textbf{Non-Apex} $=$ \texttt{other\_judiciary} $+$ \texttt{others} $+$ the remaining \texttt{sc\_judge\_congressman} IDs (Congressmen; 113 scandals). Throughout the paper ``Apex'' pools the President and Other Apex bins (63 scandals).

\autoref{tab:sup_scandal_list} lists every scandal in the panel with its scandal ID, country, year, a short description, and its Apex/Non-Apex classification. The scandal ID is the identifier used to disambiguate the leave-one-out labels in \autoref{sup:loo} when a country has more than one scandal in the same year within a group. One earlier borderline case, ID \texttt{NEW23} (Mexico, 2017), was originally re-tagged as a Supreme Court justice but its source describes a San Luis Potos\'i case implicating \emph{diputados} (members of Congress); we therefore classify it as Non-Apex. Four scandals had blank descriptions in the classification file; we recovered them from the underlying news source and report them in the table.
...
\autoref{tab:sup_democracy_index_ranks} lists the sample countries (sorted by their 2008 V-Dem Electoral Democracy Index) and shows how each is classified---High, Medium, or Low---under four different 2008 democracy measures, so the tercile grouping used throughout is not an artefact of a single index. \autoref{tab:sup_democracy_terciles_libdem} then re-estimates the tercile democracy split of \autoref{tab:sup_democracy_terciles} using an alternative continuous index, V-Dem's Liberal Democracy Index, in place of the Electoral index, and \autoref{tab:sup_democracy_terciles_libdem_lmh} reports the corresponding Low vs.\ Medium+High pooled split---the liberal-index analogue of \autoref{tab:sup_democracy_lmh}. Finally, \autoref{tab:sup_democracy_fh} repeats the by-level split using Freedom House's 2008 status (Free vs.\ Partly Free). The ordering of the effects is preserved throughout.
...
\begin{table}[htb!]
    ...
    \end{center}
    \footnotesize{Each country's High / Medium / Low classification under four 2008 democracy measures: V-Dem Electoral Democracy (\texttt{v2x\_polyarchy}), V-Dem Liberal Democracy (\texttt{v2x\_libdem}), Polity2 (\texttt{e\_polity2}), and Freedom House status (\texttt{e\_fh\_status}; Free / Partly Free / Not Free mapped to High / Medium / Low). For the three continuous indices the classification is the country's tercile; Polity2 discriminates poorly among these consolidated democracies (its scores cluster near the top), so its tercile split is noisier than the V-Dem indices. Countries are sorted by the V-Dem Electoral index.}
\end{table}
...
\begin{table}[htb!]
    ...
    \end{center}
    \footnotesize{As \autoref{tab:sup_democracy_terciles}, but the terciles are formed from V-Dem's 2008 \emph{Liberal} Democracy Index (\texttt{v2x\_libdem}) rather than the Electoral index. OLS estimates of $\beta_{\text{High}}$, $\beta_{\text{Medium}}$, and $\beta_{\text{Low}}$ (the post-scandal indicator interacted with top-, middle-, and bottom-tercile flags), on violent and peaceful protest counts at the $\pm$30/60/90/120-day windows. The rows report the one-sided $p$-values for the orderings High $>$ Medium, Medium $>$ Low, and High $>$ Low. All specifications include country-by-year, month-of-year, and day-of-week fixed effects; standard errors (in parentheses) are clustered at the country $\times$ year $\times$ day-bin level. Sample period 2008--2018; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}
...
\begin{table}[htb!]
    ...
    \end{center}
    \footnotesize{Windowed post-scandal ATT (Panel A) and the joint pre-trend $p$-value (Panel B, testing that all pre-period lead coefficients are zero) from four specifications---no adjustment (TWFE), regression adjustment (four outcome lags), inverse-propensity weighting, and doubly robust---on the country $\times$ 15-day-bin staggered panel (treatment $=$ each country's first-scandal bin; clean controls $=$ scandal-free country-years). Columns are violent and non-violent protests at the $\pm 60$/$\pm 90$/$\pm 120$-day windows. Standard errors clustered at the country level. Sample period 2008--2018; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}
...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{Violent-protest event studies overlaying the four specifications---no adjustment, regression adjustment (four outcome lags), IPW, and doubly robust---on the country $\times$ 15-day-bin staggered panel, $\pm 60$-day window; the bin immediately before disclosure ($-1$) is the reference. Vertical bars are 90\% confidence intervals; standard errors clustered at the country level. Sample period 2008--2018; Venezuela excluded.}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{Binned means of the violent-protest count---residualised on the country-by-year, month-of-year, and day-of-week fixed effects---in 5-day bins of the running variable (days since scandal), with local-linear fits on each side of the disclosure cutoff, at the $h=30$ (top row), $h=60$ (middle), and $h=90$ (bottom) bandwidths. For apex scandals (left column) the count jumps upward at $w_{s,t}=0$ at every bandwidth; for non-apex scandals (right column) the fit is continuous through the cutoff. All regression-discontinuity figures share a common vertical scale so the discontinuities can be compared by eye. Sample period 2008--2018; Venezuela excluded.}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{center}
    \footnotesize{As \autoref{fig:sup_rd_time}, but with local-\emph{quadratic} fits on each side of the disclosure cutoff, at the $h=30$ (top row), $h=60$ (middle), and $h=90$ (bottom) bandwidths. Binned means of the violent-protest count, residualised on the country-by-year, month-of-year, and day-of-week fixed effects, in 5-day bins of the running variable. For apex scandals (left column) the count jumps upward at $w_{s,t}=0$; for non-apex scandals (right column) the fit is continuous through the cutoff. Common vertical scale across all regression-discontinuity figures. Sample period 2008--2018; Venezuela excluded.}
\end{figure}

The result is robust to the polynomial choice---the local-quadratic estimates are in \autoref{tab:sup_rd_time}, and \autoref{fig:sup_rd_time_quad} plots the corresponding local-quadratic fits, which likewise jump upward at disclosure for apex scandals only---and to a fully non-parametric fit. \autoref{tab:sup_rd_time_np} reports local-polynomial regression-discontinuity estimates~\citep{calonico2014robust} at the $\pm30/60/90$-day windows, each with its own MSE-optimal, data-driven bandwidth and a triangular kernel, and \autoref{fig:sup_rd_time_np} shows the corresponding local-linear fits with 90\% confidence bands. The apex violent discontinuity is positive at every window; with the wider robust confidence bands it is not always individually significant, while the non-apex and peaceful estimates remain null throughout.
...
We re-estimate each coefficient on violent protests in the $\pm$30-day window, dropping one scandal's event window at a time, for three different splits of the scandals: by scandal type (Apex vs.\ Non-Apex, \autoref{fig:sup_loo}); by the rank hierarchy of Table~\ref{tab:sup_hierarchy} (President / Other Apex / Non-Apex, \autoref{fig:sup_loo_hier}); and by the 2008 democracy tercile of the scandal's country (High / Medium / Low, \autoref{fig:sup_loo_demo}). In every case no single scandal drives its coefficient---the leave-one-out estimates cluster tightly on the full-sample line (red). For the apex effect, removing the single most influential scandal lowers $\beta_{\text{Apex}}$ from about 0.048 to 0.029 but leaves it clearly positive. Within each split the panels share a common vertical axis so the magnitudes are directly comparable.
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{Leave-one-out estimates of the two-group interaction coefficients (\autoref{eq:interaction}) on violent protests, $\pm 30$-day window. Each point re-estimates the panel's coefficient---Post $\times$ Apex in (a), Post $\times$ Non-Apex in (b)---dropping the labelled scandal's event window; panel (a) leaves out each of the 63 apex scandals, panel (b) each of the 113 non-apex scandals. Vertical bars are 90\% confidence intervals; scandals (x-axis, Country -- Year) are sorted by the estimate. The red line is the coefficient on all scandals and the faint line marks zero; the two panels share a common vertical scale. Standard errors clustered at country $\times$ year $\times$ day-bin. Sample period 2008--2018; Venezuela excluded.}
\end{figure}
...
% ---------------------------------------------------------------------
\clearpage
\section{Synthetic difference-in-differences}\label{sup:sdid}

As a further robustness check we re-estimate the effect with synthetic difference-in-differences~\citep{arkhangelsky2021synthetic}, which reweights control units and pre-treatment periods to match the treated unit's pre-scandal trajectory before differencing. Because a country experiences many scandals over the sample---so a single absorbing ``treated'' status is ill-defined---we estimate the effect \emph{scandal by scandal} and average. For each scandal the treated unit is its country, ``adopting'' treatment at the disclosure date $t_0$; the donor pool is every other country (Venezuela excluded) with \emph{no} scandal of its own in the pre-scandal window $[t_0-K,\,t_0-1]$ that is fully observed over $[t_0-K,\,t_0+K]$ (17--19 donors qualify on average). We run this separately for the apex and non-apex partitions and for violent and peaceful protests, at windows $K\in\{30,60,90\}$ days, and report the mean of the per-scandal estimates with standard errors clustered by country.

\autoref{tab:sup_sdid} shows the results. At the $\pm 30$-day window the apex effect on violent protests is $0.039$ ($p<0.10$), close to the OLS estimate in Table~\ref{tab:main}, whereas the peaceful margin and the entire non-apex partition are null at every window. The apex violent effect attenuates at wider windows, consistent with a sharp, front-loaded mobilization. Because the synthetic control draws its counterfactual from contemporaneous protest dynamics in scandal-free countries---rather than from the treated country's own other days---this design corroborates the immediate apex$\,\rightarrow\,$violent response through a different identifying comparison. Figures~\ref{fig:sdid_trends_apex} and~\ref{fig:sdid_trends_nonapex} plot the observed and synthetic-control trajectories for the apex and non-apex partitions (violent and peaceful protests, at the $\pm$30/60/90-day windows, 15-day bins), and Figures~\ref{fig:sdid_placebo_apex} and~\ref{fig:sdid_placebo_nonapex} report the in-space placebo (permutation) test.

\begin{table}[htb!]
    \caption{Synthetic difference-in-differences: per-scandal stacked estimates.}
    \label{tab:sup_sdid}
    \begin{center}
        \scriptsize{\input{sup_sdid_stacked}}
    \end{center}
    \footnotesize{Synthetic difference-in-differences estimates~\citep{arkhangelsky2021synthetic} of the effect of a scandal disclosure on the daily count of protests (Mass Mobilization), computed per scandal and averaged. For each scandal the treated unit is its country, ``adopting'' treatment at the disclosure date; the donor pool comprises the countries with no scandal of their own in the pre-scandal window $[t_0-K,\,t_0-1]$ that are fully observed over the $\pm K$ window (Venezuela excluded). Cells report the mean across scandals of the per-scandal $\widehat{\text{ATT}}$ (the average post-disclosure gap between the treated country and its synthetic control); standard errors, in parentheses, are clustered by country so that a country's repeated scandals are not treated as independent. ``Number of scandals'' counts the treated scandals in each panel; ``Avg.\ donor countries'' is the mean donor-pool size. Sample period 2008--2018. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}

% --- SDID diagnostics, full grid: observed-vs-synthetic (trends) and the
%     in-space placebo, for {apex, non-apex} x {violent, peaceful} at the
%     +-30/60/90-day windows (15-day bins; daily/5/10-day versions are also
%     produced by a_sdid_figures_pa_vs_na.do). ---
\begin{figure}[htbp]
    \caption{Synthetic difference-in-differences, observed vs.\ synthetic-control trajectory --- Apex scandals (15-day bins).}
    \label{fig:sdid_trends_apex}
    \begin{center}
    \begin{subfigure}{0.49\textwidth}\caption{Violent, $\pm 30$.}
        \includegraphics[width=\textwidth]{sdid_trends_apex_violent_w30_b15.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Peaceful, $\pm 30$.}
        \includegraphics[width=\textwidth]{sdid_trends_apex_peaceful_w30_b15.pdf}\end{subfigure}

    \begin{subfigure}{0.49\textwidth}\caption{Violent, $\pm 60$.}
        \includegraphics[width=\textwidth]{sdid_trends_apex_violent_w60_b15.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Peaceful, $\pm 60$.}
        \includegraphics[width=\textwidth]{sdid_trends_apex_peaceful_w60_b15.pdf}\end{subfigure}

    \begin{subfigure}{0.49\textwidth}\caption{Violent, $\pm 90$.}
        \includegraphics[width=\textwidth]{sdid_trends_apex_violent_w90_b15.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Peaceful, $\pm 90$.}
        \includegraphics[width=\textwidth]{sdid_trends_apex_peaceful_w90_b15.pdf}\end{subfigure}

    \end{center}
    \footnotesize{Observed treated trajectory (solid red), the synthetic control as estimated (dashed navy), and that synthetic shifted up by its pre-scandal offset to align the pre-period (dotted green), averaged across the apex scandals in event time (daily counts pooled into 15-day bins). Synthetic difference-in-differences identifies off the \emph{change} in the treated-minus-synthetic gap, so the post-disclosure divergence of the observed line from the shifted synthetic is the estimated effect; the synthetic sits below because it aggregates many donor countries. Vertical lines mark the disclosure date. Sample period 2008--2018; Venezuela excluded.}
\end{figure}

\begin{figure}[htbp]
    \caption{Synthetic difference-in-differences, observed vs.\ synthetic-control trajectory --- Non-Apex scandals (15-day bins).}
    \label{fig:sdid_trends_nonapex}
    \begin{center}
    \begin{subfigure}{0.49\textwidth}\caption{Violent, $\pm 30$.}
        \includegraphics[width=\textwidth]{sdid_trends_nonapex_violent_w30_b15.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Peaceful, $\pm 30$.}
        \includegraphics[width=\textwidth]{sdid_trends_nonapex_peaceful_w30_b15.pdf}\end{subfigure}

    \begin{subfigure}{0.49\textwidth}\caption{Violent, $\pm 60$.}
        \includegraphics[width=\textwidth]{sdid_trends_nonapex_violent_w60_b15.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Peaceful, $\pm 60$.}
        \includegraphics[width=\textwidth]{sdid_trends_nonapex_peaceful_w60_b15.pdf}\end{subfigure}

    \begin{subfigure}{0.49\textwidth}\caption{Violent, $\pm 90$.}
        \includegraphics[width=\textwidth]{sdid_trends_nonapex_violent_w90_b15.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Peaceful, $\pm 90$.}
        \includegraphics[width=\textwidth]{sdid_trends_nonapex_peaceful_w90_b15.pdf}\end{subfigure}

    \end{center}
    \footnotesize{Observed treated trajectory (solid red), the synthetic control as estimated (dashed navy), and that synthetic shifted up by its pre-scandal offset to align the pre-period (dotted green), averaged across the non-apex scandals in event time (daily counts pooled into 15-day bins). Synthetic difference-in-differences identifies off the \emph{change} in the treated-minus-synthetic gap, so the post-disclosure divergence of the observed line from the shifted synthetic is the estimated effect; the synthetic sits below because it aggregates many donor countries. Vertical lines mark the disclosure date. Sample period 2008--2018; Venezuela excluded.}
\end{figure}

\begin{figure}[htbp]
    \caption{Synthetic difference-in-differences, in-space placebo test --- Apex scandals (15-day bins).}
    \label{fig:sdid_placebo_apex}
    \begin{center}
    \begin{subfigure}{0.49\textwidth}\caption{Violent, $\pm 30$.}
        \includegraphics[width=\textwidth]{sdid_placebo_apex_violent_w30_b15.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Peaceful, $\pm 30$.}
        \includegraphics[width=\textwidth]{sdid_placebo_apex_peaceful_w30_b15.pdf}\end{subfigure}

    \begin{subfigure}{0.49\textwidth}\caption{Violent, $\pm 60$.}
        \includegraphics[width=\textwidth]{sdid_placebo_apex_violent_w60_b15.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Peaceful, $\pm 60$.}
        \includegraphics[width=\textwidth]{sdid_placebo_apex_peaceful_w60_b15.pdf}\end{subfigure}

    \begin{subfigure}{0.49\textwidth}\caption{Violent, $\pm 90$.}
        \includegraphics[width=\textwidth]{sdid_placebo_apex_violent_w90_b15.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Peaceful, $\pm 90$.}
        \includegraphics[width=\textwidth]{sdid_placebo_apex_peaceful_w90_b15.pdf}\end{subfigure}

    \end{center}
    \footnotesize{Permutation (in-space placebo) test. Each gray line is one donor country's average placebo gap---treated minus synthetic, demeaned by its own pre-scandal average and pooled into 15-day bins---across the scandals in which it serves as a donor; the red line is the true apex gap and the black horizontal line marks a zero gap. Sample period 2008--2018; Venezuela excluded.}
\end{figure}

\begin{figure}[htbp]
    \caption{Synthetic difference-in-differences, in-space placebo test --- Non-Apex scandals (15-day bins).}
    \label{fig:sdid_placebo_nonapex}
    \begin{center}
    \begin{subfigure}{0.49\textwidth}\caption{Violent, $\pm 30$.}
        \includegraphics[width=\textwidth]{sdid_placebo_nonapex_violent_w30_b15.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Peaceful, $\pm 30$.}
        \includegraphics[width=\textwidth]{sdid_placebo_nonapex_peaceful_w30_b15.pdf}\end{subfigure}

    \begin{subfigure}{0.49\textwidth}\caption{Violent, $\pm 60$.}
        \includegraphics[width=\textwidth]{sdid_placebo_nonapex_violent_w60_b15.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Peaceful, $\pm 60$.}
        \includegraphics[width=\textwidth]{sdid_placebo_nonapex_peaceful_w60_b15.pdf}\end{subfigure}

    \begin{subfigure}{0.49\textwidth}\caption{Violent, $\pm 90$.}
        \includegraphics[width=\textwidth]{sdid_placebo_nonapex_violent_w90_b15.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Peaceful, $\pm 90$.}
        \includegraphics[width=\textwidth]{sdid_placebo_nonapex_peaceful_w90_b15.pdf}\end{subfigure}

    \end{center}
    \footnotesize{Permutation (in-space placebo) test. Each gray line is one donor country's average placebo gap---treated minus synthetic, demeaned by its own pre-scandal average and pooled into 15-day bins---across the scandals in which it serves as a donor; the red line is the true non-apex gap and the black horizontal line marks a zero gap. Sample period 2008--2018; Venezuela excluded.}
\end{figure}

% ---------------------------------------------------------------------
\clearpage
\subsection{Government violent response as the outcome}\label{sup:gvr}

As a final check we re-estimate the specifications behind the main paper's Table~1 (Apex vs.\ Non-Apex interaction), Figure~1 (subsample event study), Table~2 (main effect and benchmarks), Table~3 (democracy split, Electoral index), and Table~S3 (democracy split, Liberal index) on a \emph{different} outcome: the daily count of \emph{government violent responses} to protests. Everything else---sample, fixed effects, clustering, and windows---is unchanged. The government's violent response \emph{rises} after apex corruption scandals: in the pooled interaction the Post $\times$ Apex effect is $0.022$--$0.035$ and significant at every window, while the non-apex effect is null or slightly negative (\autoref{tab:gvr_interaction}); the event study (\autoref{fig:gvr_es}) shows no pre-trend and a post-disclosure increase concentrated in the first month. The same pattern holds in the main-effect table---the apex panel is positive while the football-loss and currency-depreciation benchmarks are null (\autoref{tab:gvr_main})---and the response is positive across the democracy split (Tables~\ref{tab:gvr_democracy_elec}--\ref{tab:gvr_democracy_lib}). Apex scandals thus move \emph{both} margins, citizen violence and the state's violent response, consistent with an escalation dynamic rather than one-sided mobilization.

\begin{table}[htb!]
    \caption{Government violent response, Apex vs.\ Non-Apex interaction (specification of main-paper Table~1).}
    \label{tab:gvr_interaction}
    \begin{center}\scriptsize{\input{gvr_interaction}}\end{center}
    \footnotesize{OLS estimates of the pooled two-group interaction (Post $\times$ Apex and Post $\times$ Non-Apex) on the daily government-violent-response count, at the $\pm$30/60/90-day windows. Same sample, fixed effects (country-by-year, month-of-year, day-of-week) and clustering (country $\times$ year $\times$ day-bin) as the main paper's Table~1; only the outcome differs. Sample period 2008--2018; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}

\begin{table}[htb!]
    \caption{Government violent response, main effect and benchmarks (specification of main-paper Table~2).}
    \label{tab:gvr_main}
    \begin{center}\scriptsize{\input{gvr_main}}\end{center}
    \footnotesize{Effect of a post-event indicator on the government-violent-response count, estimated on four samples (apex scandals, non-apex scandals, football-match losses, currency depreciations), at the $\pm$30/60/90-day windows. Construction as in the main paper's Table~2, with the outcome changed. Sample period 2008--2018; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}

\begin{table}[htb!]
    \caption{Government violent response by 2008 level of democracy, Electoral index (specification of main-paper Table~3).}
    \label{tab:gvr_democracy_elec}
    \begin{center}\scriptsize{\input{gvr_democracy_elec}}\end{center}
    \footnotesize{OLS estimates of Post $\times$ Medium+High and Post $\times$ Low---the post-scandal indicator interacted with the top-two-tercile and bottom-tercile flags of the 2008 V-Dem Electoral Democracy Index---on the government-violent-response count. Construction as in the main paper's Table~3; only the outcome differs. Sample period 2008--2018; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}

\begin{table}[htb!]
    \caption{Government violent response by 2008 level of democracy, Liberal index (specification of main-paper Table~S3).}
    \label{tab:gvr_democracy_lib}
    \begin{center}\scriptsize{\input{gvr_democracy_lib}}\end{center}
    \footnotesize{As \autoref{tab:gvr_democracy_elec}, but the terciles are formed from the 2008 V-Dem \emph{Liberal} Democracy Index. Sample period 2008--2018; Venezuela excluded. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}

\begin{figure}[htb!]
    \caption{Government violent response: subsample event study (specification of main-paper Figure~1; $\pm 60$-day window, 15-day bins).}
    \label{fig:gvr_es}
    \begin{center}
    \begin{subfigure}{0.49\textwidth}\caption{Apex.}
        \includegraphics[width=\textwidth]{gvr_es_pa.pdf}\end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}\caption{Non-Apex.}
        \includegraphics[width=\textwidth]{gvr_es_na.pdf}\end{subfigure}
    \end{center}
    \footnotesize{Event-time coefficients (points) with 90\% confidence intervals (spikes) from the subsample event study of the main paper's Figure~1, estimated on the daily government-violent-response count; the bin immediately before disclosure ($[-15,-1]$ days) is the omitted reference. Same fixed effects and clustering as Figure~1. Sample period 2008--2018; Venezuela excluded.}
\end{figure}
