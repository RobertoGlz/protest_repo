...
As a final check we re-estimate the specifications behind the main paper's Table~1 (Apex vs.\ Non-Apex interaction), Figure~1 (subsample event study), Table~2 (main effect and benchmarks), Table~3 (democracy split, Electoral index), and Table~S3 (democracy split, Liberal index) on a \emph{different} outcome: the daily count of \emph{government violent responses} to protests from the Mass Mobilization project, merged into the event-window panel by country and date. Everything else---sample, fixed effects, clustering, and windows---is unchanged. In sharp contrast to violent protests, the government's violent response does \emph{not} increase after apex corruption scandals: across every specification and window the estimates are essentially zero and, where significant, slightly \emph{negative}, and the event study (\autoref{fig:gvr_es}) shows no post-disclosure dynamics. The apex effect, though tiny, is consistently \emph{less} negative than the non-apex effect (\autoref{tab:gvr_interaction}), and the (negative) response is concentrated in the lower-democracy countries (Tables~\ref{tab:gvr_democracy_elec}--\ref{tab:gvr_democracy_lib}).
