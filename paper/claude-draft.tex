...
% =====================================================================
\begin{abstract}
    \noindent We document that \textit{apex} corruption scandals---corrupt acts implicating top-level politicians---cause immediate and persistent increases in violent protests. Using a panel of 176 corruption scandals across 23 countries in Latin America from 2008 to 2018, merged with daily protest counts, we show that the daily count of violent protests rises sharply after scandals implicating a President or Other Apex official---by about 0.06 protests per country-day in the month following disclosure, roughly 1.7 times the pre-scandal baseline---while scandals implicating non-apex officials produce no such immediate response, and a pooled test rejects equality of the two effects. The response appears within a week of disclosure and persists for months. It is specific to apex corruption: neither football match losses nor currency depreciations, two other high-salience national shocks, move violent protests. Our findings identify a high-frequency, high-magnitude channel through which corruption shapes the political environment in democracies.
\end{abstract}
...
Each of the 176 scandals is manually classified by the political rank of the implicated official into three bins: \textbf{President} (45 scandals); \textbf{Other Apex}---Governors and Supreme Court Justices (19 scandals); and \textbf{Other Non-Apex}---Congressmen, lower judiciary, and other officials (112 scandals). Throughout, we group the President and Other Apex scandals together (``apex''; 64 scandals) and contrast them with the Other Non-Apex scandals.
...
\end{equation*}
where $\mathrm{PA}_s$ ($\mathrm{NA}_s$) indicates a President + Other Apex (Other Non-Apex) scandal, and the row ``$p$-value: (Pres+OA) $=$ (ONA)'' reports the two-sided $p$-value for $H_0:\beta_{\text{PA}}=\beta_{\text{NA}}$. In this pooled specification the immediate ($\pm$30-day) violent-protest effect is 0.048 for apex scandals ($p<0.01$) versus $-0.003$ for non-apex scandals, and we reject equality of the two ($p = 0.020$). At the $\pm$120-day horizon both are positive (0.022 for apex, $p<0.05$; 0.012 for non-apex) and no longer statistically distinguishable ($p = 0.411$). The \emph{immediate} violent response to corruption is thus concentrated among apex scandals.

\begin{table}[htb!]
    \caption{Pooled test: (President + Other Apex) vs.\ Other Non-Apex.}
    \label{tab:interaction}
    \begin{center}
        \scriptsize{\input{sup_interaction_pa_vs_na}}
    \end{center}
    \footnotesize{OLS estimates of $\beta_{\text{PA}}$ and $\beta_{\text{NA}}$ from the pooled specification above, on the same eight outcome--window cells as Table~\ref{tab:main}. The sample is restricted to classified scandals (President + Other Apex or Other Non-Apex). The row ``$p$-value: (Pres+OA) $=$ (ONA)'' reports the two-sided $p$-value for $H_0:\beta_{\text{PA}}=\beta_{\text{NA}}$. Standard errors (in parentheses) are clustered at the country $\times$ year $\times$ day-bin level. Same fixed effects and sample period as Table~\ref{tab:main}. Significance: $^{*}$ $p<0.1$, $^{**}$ $p<0.05$, $^{***}$ $p<0.01$.}
\end{table}

\subsection{Protest dynamics around apex corruption scandals}

\autoref{fig:dynamics} traces the dynamics of the effect, replacing the post-scandal indicator with a fully saturated set of 30-day event-time bin indicators over the $\pm 120$-day window (reference bin: the 30 days immediately preceding disclosure), estimated separately on the two subsamples of Table~\ref{tab:main}. In the President + Other Apex subsample, violent protests (\autoref{fig:dynamics:pav}) show no pre-trend and then jump at disclosure, remaining elevated in the months that follow; peaceful protests (\autoref{fig:dynamics:pap}) are flat throughout. In the Other Non-Apex subsample, neither violent (\autoref{fig:dynamics:nav}) nor peaceful (\autoref{fig:dynamics:nap}) protests depart from their pre-scandal path. The absence of a pre-trend in every panel supports the identifying assumption that the within-year timing of disclosure is as good as random relative to the protest baseline.
...
Is the violent-protest response a general reaction to salient national news, or is it specific to apex corruption? \autoref{tab:benchmarks} benchmarks the corruption effect against two other high-salience national shocks whose timing is plausibly as good as random with respect to the protest baseline: national-team football (soccer) match losses and currency depreciation episodes. Each shock is estimated with exactly the Table~\ref{tab:main} specification and the same eight outcome--window columns (counts and shares). The table has four panels so that both headline corruption regressions (President + Other Apex; Other Non-Apex) can be read directly against the two benchmarks. The apex-corruption effect on violent protests in the $\pm$30-day window (0.060, $p<0.01$) is an order of magnitude larger than---and statistically unlike---the corresponding football (0.006) and currency-depreciation (0.004) estimates, neither of which is distinguishable from zero. The mobilization response is thus specific to apex corruption, not a generic reaction to salient national events.
...
Does the violent-protest response depend on the strength of a country's democratic institutions? \autoref{tab:democracy} splits the panel countries at the median of their 2008 level of democracy---measured by V-Dem's Electoral Democracy Index (\texttt{v2x\_polyarchy})~\citep{vdem2024}---and interacts the post-scandal indicator with the resulting above-/below-median classification. The time-invariant level of democracy is absorbed by the country-by-year fixed effects, so only the $\mathrm{Post}\times$ interactions are identified. The violent-protest response is positive in both groups but, if anything, larger and more persistent in the \emph{above}-median (higher-democracy) countries: the $\pm$30-day effect is 0.021 ($p<0.05$) in higher-democracy countries versus 0.009 ($p<0.1$) in lower-democracy ones, and at the $\pm$120-day horizon it is 0.024 ($p<0.01$) versus 0.005 (not significant). The row ``$p$-value: HIGH $=$ LOW'' shows, however, that we cannot statistically distinguish the two ($p = 0.245$ and $0.115$ for the $\pm$30- and $\pm$120-day violent-count effects), so this heterogeneity is suggestive rather than conclusive. One reading is that citizens in more established democracies hold higher expectations of accountability, so apex corruption is a starker violation; another is simply that protest is more freely expressed---and more completely recorded---where democratic institutions are stronger.
...
We re-estimate the headline OLS and Poisson QML specifications at every event-window width $T \in \{15, 30, 45, \dots, 150\}$ days and plot the point estimate against $T$ with a 90\% confidence interval, separately on the President + Other Apex and Other Non-Apex subsamples. Figures~\ref{fig:sup_window_pa} and \ref{fig:sup_window_na} report the four panels each (OLS $\hat\beta$ and Poisson QML incidence-rate ratio, for violent and peaceful protests). Within each figure the violent and peaceful panels share a common y-axis range so the null line is aligned and magnitudes are directly comparable. The two widths reported in Table~\ref{tab:main} ($T=30$ and $T=120$) are highlighted; the effect on violent protests in the President + Other Apex subsample is positive across the full range of windows.
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{For each $T \in \{15, 30, \dots, 150\}$ we re-estimate the headline specification on the event-window panel restricted to $|w_{st}|\leq T$, on the 64 President + Other Apex scandals. Vertical bars are 90\% confidence intervals; the markers at $T=30$ and $T=120$ are highlighted. Y-axis shared between the violent and peaceful panels within each estimator. Sample period 2008--2018; Venezuela excluded.}
\end{figure}
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \end{center}
    \footnotesize{Same construction as \autoref{fig:sup_window_pa}, restricted to the 112 Other Non-Apex scandals. Y-axis shared between the violent and peaceful panels within each estimator. Sample period 2008--2018; Venezuela excluded.}
\end{figure}

% ---------------------------------------------------------------------
\clearpage
\section{Effect by 2008 level of democracy: Poisson QML}\label{sup:democracy_poi}

Table~\ref{tab:sup_democracy_poi} reports the Poisson QML incidence-rate-ratio counterpart to the OLS democracy split of Table~\ref{tab:democracy} (main text), on the four count outcomes.
...
\begin{table}[htb!]
    ...
    \end{center}
    \footnotesize{Poisson QML incidence-rate ratios $\exp(\beta)$ for the two interaction terms (Post $\times$ above-/at-or-below-median 2008 democracy), count outcomes only. Countries are split at the median of V-Dem's \texttt{v2x\_polyarchy} in 2008 (16 panel countries; Venezuela excluded). ``$p$-value: HIGH $=$ LOW'' tests $H_0:\beta_{\text{High}}=\beta_{\text{Low}}$. Same fixed effects, clustering, and sample as Table~\ref{tab:democracy}.}
\end{table}
