...
/* Loop over subsamples of data based on level of politician */
forvalues lll = 1/3 {
	...
	if `lll' == 1 {
		...
		local suffix = "ministers"
	}
	else {
		local suffix = "otheragents"
	}
	...
	foreach numbdays in /*90*/ 120 {
		...
		foreach outcome in ${outcome_list} {
			...
			eststo m_`y_counter'_`numbdays' : ppmlhdfe `outcome' post ///
				if year >= `firstyear' & official_involved == `lll', absorb(month day ${CY_fe}) vce(cluster group_cluster) irr
			...
			quietly {
				...
				ppmlhdfe `outcome' ${leads} ${lags} ///
					if year >= `firstyear' & official_involved == `lll', ///
			...
			graph export ///
			"${work}/results/figures/ppmlhdfe_es_`outcome'_`numbdays'window_since`firstyear'_overlaps_`ci_level'ci_`suffix'.png", ///
				replace
		}
		/* Export table */
