/* ----------------------------------------------------------------------------
    ...
    Objective:
        Split version of a_did_modern.do.  Same country-week panel and
        same four modern staggered-DiD estimators (OLS TWFE, dCDH, BJS,
        SA), but run SEPARATELY on the two apex-partition subsamples
        used in the main Table~1:
            - pa : President + Other Apex   scandals
            - na : Other Non-Apex           scandals

        For each subsample we redefine the country-level
        `first_scandal_date` (and derived variables) using only
        scandals belonging to that subsample; countries with no
        subsample scandal become never-treated for that pass.

    Outputs (paper/{tables,figures}/):
        - did_modern_main_<sample>.tex          (per-subsample results table)
        - did_modern_es_<outcome>_<sample>.pdf  (per-outcome event study)
...
foreach sample in pa na {
	...
	di as result _newline _newline ///
		"=================================================================" _n ///
		"   DID MODERN (country-week)  --  subsample = `sample'" _n ///
		"================================================================="

	/* ---------- Reload the country-week panel and override the
	   treatment date with the subsample-specific first scandal. ---------- */
	...
	/* Integer week index (anchor on the data's first week) */
	quietly summarize week_start
	local week0 = r(min)
	capture drop week_idx
	gen long week_idx = (week_start - `week0') / 7 + 1
	label variable week_idx "Week index (1 = first week in the sample)"
	assert week_idx >= 1

	/* First-scandal week per country (only for still-treated units) */
	capture drop _fs_week_start
	capture drop first_scandal_week_idx
	...
	capture drop ever_treated D cohort I_never_treated
	...
	/* Event-time dummies (leads -4..-2, lag omitted at -1, lags 0..7) */
	capture drop etime
	...
	foreach y of local outcomes {
		...
		preserve
			...
			graph export "${figout}/did_modern_es_`y'_`sample'.pdf", replace
		restore
	}
