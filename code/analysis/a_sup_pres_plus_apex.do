...
esttab _all using "${tables}/sup_pres_plus_apex_poi_temp.tex", ///
...
tempfile poi_padded
tempname pin pout
file open `pin'  using "${tables}/sup_pres_plus_apex_poi_temp.tex", read  text
...
file close `pin'
file close `pout'
capture erase "${tables}/sup_pres_plus_apex_poi_temp.tex"

panelcombine, ///
	use("${tables}/sup_pres_plus_apex_ols_temp.tex" ///
	    "`poi_padded'") ///
	paneltitles("OLS" "Poisson QML") ///
	columncount(9) ///
	save("${tables}/sup_pres_plus_apex_table.tex") ///
	cleanup

display in green "a_sup_pres_plus_apex.do finished OK"
