...
foreach ptype in "protests" "peaceful" "violent" {
	...
	generate raw_ind_`ptype' = (raw_num_`ptype'_MM > 0) if !missing(raw_num_`ptype'_MM)
}

replace raw_government_response_violent = 0 if _merge == 2
generate raw_ind_gvt_violent = (raw_government_response_violent > 0) if !missing(raw_government_response_violent)

/* Coded outcomes in analysis data */
foreach ptype in "protests" "peaceful" "violent" {
	generate ind_`ptype' = (num_`ptype'_MM > 0) if !missing(num_`ptype'_MM)
}

generate ind_gvt_violent = (government_response_violent > 0) if !missing(government_response_violent)
...
global outcome_list = "ind_violent raw_ind_violent ind_peaceful raw_ind_peaceful ind_gvt_violent raw_ind_gvt_violent"
...
foreach numbdays in /*90*/ 120 {
	...
	foreach outcome in ${outcome_list} {
		...
		graph export ///
		"${work}/results/figures/ppmlhdfe_es_`outcome'_`numbdays'window_since`firstyear'_overlaps_`ci_level'ci_rawmmdata_indicators.png", ///
			replace
	}
	/* Export table */
