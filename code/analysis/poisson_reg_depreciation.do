...
foreach numbdays in 90 120 {
	...
	foreach outcome in ${outcome_list} {
		...
		graph export ///
		"${work}/results/figures/poisson_es_depreciation_`outcome'_`numbdays'window_since`firstyear'_overlaps_`ci_level'ci.png", ///
			replace
	}
	/* Export table */
	esttab _all using Panel_`numbdays'_depreciation.tex, replace b(3) se(3) booktabs ///
