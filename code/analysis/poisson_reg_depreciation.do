...
foreach numbdays in 30 120 {
	...
	foreach outcome in ${outcome_list} {
		...
		local ++y_counter
		/* Static (average) effect -- MATCHES the corruption Table~1
		   Poisson panel: ppmlhdfe with country x year, month and
		   day-of-week absorbed FEs, clustered SEs at country x year
		...
		coefplot, keep(${leads} ${lags}) eform(${leads} ${lags}) ///
			...
			yline(1, lwidth(thin) lpattern(shortdash) lcolor(black)) ///
		...
		graph export ///
		"${figout}/poisson_es_depreciation_`outcome'_`numbdays'window_since`firstyear'_overlaps_`ci_level'ci.png", ///
			replace
	}
	/* Export table */
	esttab _all using "${tabout}/Panel_`numbdays'_depreciation.tex", replace b(3) se(3) booktabs ///
