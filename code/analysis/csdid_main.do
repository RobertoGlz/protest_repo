/* ----------------------------------------------------------------------------
	...
	Objective: Estimate event-study specification with imputation estimator
...
/* Specify number of bootstrap repetitions to make */
local breps = 20
...
foreach numbdays in `window_length' {
	...
	foreach outcome in ${outcome_list} {
		...
		eststo m_`outcome_counter' : csdid `outcome' i.`fes', ivar(`panelvar') time(`timevar') gvar(`treat_cohort') ///
			long asinr method(dripw) cluster(country_id) level(`ci_level') ///
			agg(event) notyet
		...
		event_plot, plottype(scatter) ciplottype(rspike) lag_opt(color(maroon)) lag_ci_opt(color(maroon%20)) ///
			...
			graph_opt(xline(0, lcolor(black%10) lpattern(solid) lwidth(thick)) ///
				...
				legend(off) xlabel(, nogrid labsize(medlarge)) ylabel(, labsize(medlarge) format(%3.2fc)))
		...
		preserve
			...
			graph export "${work}/results/figures/bjs_`outcome'_`window'_cxyfe_aggregated.pdf", replace
		restore
		*/
