...
foreach numbdays in 90 120 {
	...
	keep if window == 0
		...
		generate overlap = 0
		bysort country : replace overlap = 1 if ///
			((window_end[_n-1] >= window_start[_n]) & (window_end[_n-1] <= window_end[_n]))
		sort country date_0
		keep id overlap
	...
	foreach outcome in ${outcome_list} {
		...
		graph export ///
		"${work}/results/figures/poisson_es_depreciation_`outcome'_`numbdays'window_since`firstyear'_no_overlaps_`ci_level'ci.png", ///
			replace
	}
	/* Export table */
