/* ----------------------------------------------------------------------------
	...
	Objective (Task 1, Design 1 -- "Country panel + non-scandal controls"):
		Reproduce the Table 16 estimand with modern staggered-DiD
		estimators.  THE OBSERVATION IS A COUNTRY x 3-DAY BIN -- the same
		3-day grouping used in the Latinobarometer modern-DiD code
		(all_estimators_no_dcdh_samesamp.do, time_bins = floor(./3)), so
		the estimates are comparable across the two projects.

	Panel:
		Unit  = country (23 LAC).  Time = 3-day calendar bin.
		Treatment = country's FIRST scandal; treated from its first-scandal
		        bin onward.  Never-treated cohort = the countries with no
		        scandal in 2008+.
		Outcomes = mm_protests, mm_violent, mm_nonviolent, mm_gvr
		        (3-day-bin sums).
		Event-study window = +-10 bins (+-30 days); bin -1 = reference.

	Estimators: OLS two-way FE, dCDH, BJS, SA.

	Outputs:
		${tabout}/did_modern_cp_main.tex
		${figout}/did_modern_cp_es_<outcome>.pdf
---------------------------------------------------------------------------- */
...
/* ============================================================
   1.  LOAD DAILY PANEL, COLLAPSE TO COUNTRY x 3-DAY BIN
   ============================================================ */
...
label variable D "Post first-scandal"
...
di as result "=== Country x 3-day-bin panel ==="
count
preserve
	bysort country_id: keep if _n == 1
	count
	di "Countries: " r(N)
	count if ever_treated == 1
	di "  treated:       " r(N)
	count if ever_treated == 0
	di "  never-treated: " r(N)
restore

/* ============================================================
   3.  EVENT-STUDY DUMMIES (+-10 bins, omit bin -1 = reference)
   Plot grid = 20 columns, event bins -10..-1,0..9.
   Matrix column j  <->  event bin (j - 11):
       col 1..10  = bins -10..-1   (col 10 = reference, left 0)
       col 11..20 = bins 0..9
   ============================================================ */
forvalues k = 2/10 {
...
foreach y of local outcomes {
	...
	if _rc == 0 {
		...
		/* static = equally weighted avg of the 10 post bins */
	...
	preserve
		...
		graph export "${figout}/did_modern_cp_es_`y'.pdf", replace
	restore
}

/* ============================================================
   5.  4 x 4 RESULTS TABLE  ->  did_modern_cp_main.tex
   ============================================================ */
capture file close _tbl
file open _tbl using "${tabout}/did_modern_cp_main.tex", write replace
file write _tbl "{" _n
...
file write _tbl "}" _n
file close _tbl
display in green "did_modern_cp_main.tex written"

di as result "=== Country x 3-day-bin panel: post-treatment coefficient (se) ==="
foreach er in ols dcdh bjs sa {
	local row "`er'   "
	foreach oc of numlist 1/4 {
		local b = b_`er'_`oc'
...
display in green "a_did_modern_countrypanel.do finished OK"
