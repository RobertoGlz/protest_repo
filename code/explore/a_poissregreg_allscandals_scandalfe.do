...
foreach outcome in "num_protests_MM" "num_violent_MM" "num_peaceful_MM" "government_response_violent" {
	local ++mmm
	/* No overlaps */
	eststo m`mmm' : ppmlhdfe `outcome' post if year>=2008, absorb(month day $fe3) vce(${CLUSTER2}) 
	...
	ppmlhdfe `outcome' ${leads} ${lags} if year>=2008, absorb(month day $fe3) vce(${CLUSTER2}) irr
	...
	graph export "${work}/results/figures/poissreg_es_`outcome'_`window_days'd_`cilevel'ci_scandalfe.png", replace

}

esttab _all using "${work}/results/tables/poissreg_es_`window_days'd_`cilevel'ci_scandalfe.tex", ///
	...
	label("Incidence Rate Ratio - exp(\beta)" "\midrule Observations" "Number of Scandals" "Pseudo R-squared" "Scandal FE" "SE Cluster") ///
