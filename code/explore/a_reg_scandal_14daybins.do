...
forvalues ddd = 1/12 {
	generate s_lead`=`ddd'*14' = (window >= -14*`ddd' & window < -14*`=`ddd'-1') 
	generate s_lag`=`ddd'*14' = (window <= 14*`ddd' & window > -14*`=`ddd'-1')  
}
forvalues kkk = 14(14)84 {
	label variable s_lag`kkk' "`kkk'"
}
forvalues kkk = -84(14)-14 {
	label variable s_lead`=-1*`kkk'' "`kkk'"
}

replace s_lead14 = 0 // Base category
...
xtitle("Days around scandal") xscale(titlegap(2)) xline(6.5, lcolor(black))  ///
