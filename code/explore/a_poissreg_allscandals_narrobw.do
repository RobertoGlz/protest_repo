...
foreach outcome in "num_protests_MM" "num_violent_MM" "num_peaceful_MM" "government_response_violent" {
	local ++mmm
	/* No overlaps */
	eststo m`mmm' : ppmlhdfe `outcome' post if year>=2008, absorb(month day $fe1) vce(${CLUSTER2}) 
	...
	ppmlhdfe `outcome' ib`basegroup'.event_bin_enc if year>=2008, absorb(month day $fe1) vce(${CLUSTER2}) irr

	coefplot, keep(*event_bin_enc) eform(*event_bin_enc) levels(`cilevel') ///
	baselevels omitted vertical ///
	xtitle("Days around scandal") xscale(titlegap(2)) xline(`xlinepos', lcolor(black))  ///
	yline(1, lwidth(vvvthin) lpattern(dash) lcolor(black)) ylabel(, labsize(medlarge)) xlabel(, labsize(medlarge)) ///
	...
	graph export "${work}/results/figures/poissreg_es_`outcome'_`window_days'd_`cilevel'ci.png", replace

}

esttab _all using "${work}/results/tables/poissreg_es_`window_days'd_`cilevel'ci.tex", ///
