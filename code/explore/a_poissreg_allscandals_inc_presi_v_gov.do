...
generate inc_presi_v_gov = .
replace inc_presi_v_gov = 1 if (position == "president" & political_affiliation == "incumbent")
replace inc_presi_v_gov = 0 if (position == "governor" & political_affiliation == "incumbent")
tabulate inc_presi_v_gov
...
foreach rrr in 1 0 {
	...
	foreach outcome in "num_protests_MM" "num_violent_MM" "num_peaceful_MM" "government_response_violent" {
		local ++mmm
		/* No overlaps */
		eststo m`mmm' : ppmlhdfe `outcome' post if year>=2008 & `posvar' == `rrr', absorb(month day $fe1) vce(${CLUSTER2}) 
		...
		ppmlhdfe `outcome' ${leads} ${lags} if year>=2008 & `posvar' == `rrr', absorb(month day $fe1) vce(${CLUSTER2}) irr
	
		coefplot, keep(${leads} ${lags}) eform(${leads} ${lags}) levels(`cilevel') ///
		...
		graph export "${work}/results/figures/poissreg_es_`outcome'_`window_days'd_`cilevel'ci_`posvar'`rrr'.png", replace
...
} // end loop over category restriction
...
panelcombine, use(Panel1.tex Panel0.tex) ///
	columncount(3) paneltitles("Incumbent Presidents" "Incumbent Governors") ///
	save("${work}/results/tables/poissreg_es_`window_days'd_`cilevel'ci_`posvar'.tex") cleanup
