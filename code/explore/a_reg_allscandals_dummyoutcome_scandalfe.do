...
generate dum_protests_MM = (num_protests_MM > 0) if !missing(num_protests_MM)
generate dum_violent_MM = (num_violent_MM > 0) if !missing(num_violent_MM)
generate dum_peaceful_MM = (num_peaceful_MM > 0) if !missing(num_peaceful_MM)
generate dum_gvt_violent = (government_response_violent > 0) if !missing(government_response_violent)

local cilevel = 90
/* Estimate regression and plot coefficients */
estimates clear
local mmm = 0
foreach outcome in "dum_protests_MM" "dum_violent_MM" "dum_peaceful_MM" "dum_gvt_violent" {
	local ++mmm
	/* No overlaps */
	eststo m`mmm' : reghdfe `outcome' post i.month i.day if year>=2008, absorb($fe3) vce(${CLUSTER2})
	...
	reghdfe `outcome' ${leads} ${lags} i.month i.day if year>=2008, absorb($fe3) vce(${CLUSTER2})
	...
	graph export "${work}/results/figures/es_dummy_outcome_allscandals_`outcome'_`window_days'd_`cilevel'ci_scandalfe.png", replace

}

esttab _all using "${work}/results/tables/es_dummyoutcome_allscandals_`window_days'd_`cilevel'ci_scandalfe.tex", ///
	...
	label("Observations" "Number of Scandals" "R-squared" "Scandal FE" "SE Cluster") ///
