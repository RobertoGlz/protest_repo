...
foreach outcome in "dum_protests_MM" "dum_violent_MM" "dum_peaceful_MM" "dum_gvt_violent" {
	...
	graph export "${work}/results/figures/es_dummy_outcome_`outcome'_`window_days'd_firstscandal_`cilevel'ci.png", replace

}

esttab _all using "${work}/results/tables/es_dummyoutcome_`window_days'd_firstscandal_`cilevel'ci.tex", ///
	replace booktabs nonotes nogaps b(3) se(3) ///
	mtitles("\shortstack{Protests > 0}" "\shortstack{Violent\\Protests > 0}" "\shortstack{Non-Violent\\Protests > 0}" ///
